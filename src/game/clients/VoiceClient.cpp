...
void Host(ChatClient& client) {
    ...
    EnableWindow(client.connect, FALSE);
    EnableWindow(client.host, FALSE);
    if (!client.network.Host(selectedPort, "Ocarina of Time")) {
...
int WINAPI WinMain(HINSTANCE instance, HINSTANCE, LPSTR commandLine, int showCommand) {
    ...
    windowClass.lpszClassName = "VoiceChatClient";
    ...
    Error("Voice client window class registered");
    ...
    HWND window = CreateWindowExA(0, windowClass.lpszClassName, "Voice and Text", style, CW_USEDEFAULT,
