...
MultiplayerHudRenderer::MultiplayerHudRenderer(Engine::Rendering::GameRenderer& renderer) : mRenderer(renderer) {
}
...
void MultiplayerHudRenderer::DrawChat(const MultiplayerHudView& view) {
    ...
    const float windowWidth = static_cast<float>(mRenderer.GetWidth());
    const float width =
    ...
    Engine::Rendering::Overlay::QueueRect(
    ...
    for (size_t index = first; index < newest; ++index) {
        ...
        Engine::Rendering::Overlay::QueueText(
            line.c_str(), x + 8.0f,
...
void MultiplayerHudRenderer::DrawLife(const MultiplayerHudView& view) {
    ...
    const float windowHeight = static_cast<float>(mRenderer.GetHeight());
