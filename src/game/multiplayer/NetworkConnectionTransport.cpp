...
bool NetworkConnectionTransport::Connect(const std::string& address,
                                         uint16_t port, const std::string& playerName) {
    if (IsActive() || address.empty() || port == 0 || port > 49151 || playerName.empty()) return false;
    gCancelConnection.store(false, std::memory_order_relaxed);
    mConnectFuture = std::async(
        std::launch::async, [address, port, playerName]() {
            ConnectAttempt attempt;
            unsigned short selectedPort = port;
            attempt.client.reset(CreateNetClient());
            attempt.result = attempt.client
                ? attempt.client->Init(address, "", false, selectedPort, playerName,
