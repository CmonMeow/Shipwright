...
constexpr size_t kResourceHeaderSize = 64;
...
bool ServerCollisionWorld::LoadArchive(const std::filesystem::path& archivePath) {
    ...
    for (zip_uint64_t entryIndex = 0; entryIndex < static_cast<zip_uint64_t>(entryCount); ++entryIndex) {
        ...
        zip_stat_t stat{};
        if (zip_stat_index(archive, entryIndex, 0, &stat) != 0 || stat.size <= kResourceHeaderSize ||
        ...
        if (!reader.Seek(kResourceHeaderSize) || !reader.Skip(12)) {
            continue;
        }
