...
static void Mp3DecoderWorker(std::shared_ptr<SOH::AudioSample> audioSample, std::shared_ptr<Engine::File> sampleFile) {
    drmp3 mp3;
...
static void FlacDecoderWorker(std::shared_ptr<SOH::AudioSample> audioSample, std::shared_ptr<Engine::File> sampleFile) {
...
static void OggDecoderWorker(std::shared_ptr<SOH::AudioSample> audioSample, std::shared_ptr<Engine::File> sampleFile,
...
std::shared_ptr<Engine::IResource>
ResourceFactoryXMLAudioSampleV0::ReadResource(std::shared_ptr<Engine::File> file,
                                              std::shared_ptr<Engine::ResourceInitData> initData) {
    if (!FileHasValidFormatAndReader(file, initData)) {
        return nullptr;
    }
    ...
    auto sampleFile = Engine::Context::GetInstance()->GetResourceManager()->GetArchiveManager()->LoadFile(path);
