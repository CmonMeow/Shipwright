...
ResourceFactoryBinaryAudioSoundFontV2::ReadResource(std::shared_ptr<Engine::File> file,
    ...
    for (uint32_t i = 0; i < audioSoundFont->soundFont.numDrums; i++) {
        ...
        if (sampleFileName.empty()) {
            ...
            auto res = Engine::Context::GetInstance()->GetResourceManager()->LoadResourceProcess(sampleFileName.c_str());
...
void ResourceFactoryXMLSoundFontV0::ParseDrums(AudioSoundFont* soundFont, tinyxml2::XMLElement* element) {
    ...
    do {
        ...
        if (sampleStr != nullptr && sampleStr[0] != 0) {
            auto res = Engine::Context::GetInstance()->GetResourceManager()->LoadResourceProcess(sampleStr);
            drum->sound.sample = static_cast<Sample*>(res ? res->GetRawPointer() : nullptr);
        } else {
            drum->sound.sample = nullptr;
        }
        ...
        if (instrumentElement != nullptr && !strcmp("LowNotesSound", instrumentElement->Name())) {
            ...
            if (sampleStr != nullptr && sampleStr[0] != 0) {
                std::shared_ptr<SOH::AudioSample> res = static_pointer_cast<SOH::AudioSample>(
                    Engine::Context::GetInstance()->GetResourceManager()->LoadResourceProcess(sampleStr));
                if (res->tuning != -1.0f) {
...
std::shared_ptr<Engine::IResource>
ResourceFactoryXMLSoundFontV0::ReadResource(std::shared_ptr<Engine::File> file,
                                            std::shared_ptr<Engine::ResourceInitData> initData) {
    if (!FileHasValidFormatAndReader(file, initData)) {
        return nullptr;
    }
    ...
    std::shared_ptr<Engine::IResource> sf;
    ...
    if (patch != nullptr) {
        ...
        audioSoundFont = dynamic_pointer_cast<AudioSoundFont>(
            Engine::Context::GetInstance()->GetResourceManager()->LoadResourceProcess(origName));
    } else {
