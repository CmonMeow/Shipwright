...
std::shared_ptr<Engine::IResource>
ResourceFactoryXMLSkeletonV0::ReadResource(std::shared_ptr<Engine::File> file,
                                           std::shared_ptr<Engine::ResourceInitData> initData) {
    if (!FileHasValidFormatAndReader(file, initData)) {
        return nullptr;
    }
    ...
    while (child != nullptr) {
        ...
        if (childName == "SkeletonLimb") {
            ...
            auto limb = Engine::Context::GetInstance()->GetResourceManager()->LoadResourceProcess(limbName.c_str());
