...
template <typename T> static void WriteInsnOneArg(Engine::BinaryWriter* writer, uint8_t opcode, T arg) {
...
static void WriteInsnTwoArg(Engine::BinaryWriter* writer, uint8_t opcode, T1 arg1, T2 arg2) {
...
static void WriteInsnThreeArg(Engine::BinaryWriter* writer, uint8_t opcode, T1 arg1, T2 arg2, T3 arg3) {
    static_assert(std::is_fundamental<T1>::value && std::is_fundamental<T2>::value);
    writer->Write(opcode);
    writer->Write(arg1);
    writer->Write(arg2);
...
static void WriteInsnNoArg(Engine::BinaryWriter* writer, uint8_t opcode) {
    writer->Write(opcode);
}

static void WriteLegato(Engine::BinaryWriter* writer) {
    WriteInsnNoArg(writer, 0xC4);
}

static void WriteNoLegato(Engine::BinaryWriter* writer) {
    WriteInsnNoArg(writer, 0xC5);
}

static void WriteMuteBhv(Engine::BinaryWriter* writer, uint8_t arg) {
    WriteInsnOneArg(writer, 0xD3, arg);
}

static void WriteMuteScale(Engine::BinaryWriter* writer, uint8_t arg) {
    WriteInsnOneArg(writer, 0xD5, arg);
}

static void WriteInitchan(Engine::BinaryWriter* writer, uint16_t channels) {
    WriteInsnOneArg(writer, 0xD7, channels);
}

static void WriteLdchan(Engine::BinaryWriter* writer, uint8_t channel, uint16_t offset) {
    WriteInsnOneArg(writer, 0x90 | channel, offset);
}

static void WriteVolSHeader(Engine::BinaryWriter* writer, uint8_t vol) {
    WriteInsnOneArg(writer, 0xDB, vol);
}

static void WriteVolCHeader(Engine::BinaryWriter* writer, uint8_t vol) {
    WriteInsnOneArg(writer, 0xDF, vol);
}

static void WriteTempo(Engine::BinaryWriter* writer, uint8_t tempo) {
    WriteInsnOneArg(writer, 0xDD, tempo);
}

static void WriteJump(Engine::BinaryWriter* writer, uint16_t offset) {
    WriteInsnOneArg(writer, 0xFB, offset);
}

static void WriteDisablecan(Engine::BinaryWriter* writer, uint16_t channels) {
    WriteInsnOneArg(writer, 0xD6, channels);
}

static void WriteNoshort(Engine::BinaryWriter* writer) {
    WriteInsnNoArg(writer, 0xC4);
}

static void WriteLdlayer(Engine::BinaryWriter* writer, uint8_t layer, uint16_t offset) {
    WriteInsnOneArg(writer, 0x88 | layer, offset);
}

static void WritePan(Engine::BinaryWriter* writer, uint8_t pan) {
    WriteInsnOneArg(writer, 0xDD, pan);
}

static void WriteBend(Engine::BinaryWriter* writer, uint8_t bend) {
    WriteInsnOneArg(writer, 0xD3, bend);
}

static void WriteInstrument(Engine::BinaryWriter* writer, uint8_t instrument) {
    WriteInsnOneArg(writer, 0xC1, instrument);
}

static void WriteTranspose(Engine::BinaryWriter* writer, int8_t transpose) {
    WriteInsnOneArg(writer, 0xC2, transpose);
}

static void WriteDelay(Engine::BinaryWriter* writer, uint16_t delay) {
    if (delay > 0x7F) {
...
template <typename T> static void WriteLDelay(Engine::BinaryWriter* writer, T delay) {
    WriteInsnOneArg(writer, 0xC0, delay);
}

template <typename T> static void WriteNotedv(Engine::BinaryWriter* writer, uint8_t note, T delay, uint8_t velocity) {
    WriteInsnTwoArg(writer, note, delay, velocity);
}

static void WriteNotedvg(Engine::BinaryWriter* writer, uint8_t note, uint16_t delay, uint8_t velocity, uint8_t gateTime) {
    if (delay > 0x7F) {
...
static void WriteMonoSingleSeq(Engine::BinaryWriter* writer, uint16_t delay, uint8_t tempo, bool looped) {
    ...
    writer->Seek(channelPlaceholderOff, Engine::SeekOffsetType::Start);
    WriteLdchan(writer, 0, channelStart);
    writer->Seek(channelStart, Engine::SeekOffsetType::Start);
    ...
    writer->Seek(layerPlaceholderOff, Engine::SeekOffsetType::Start);
    WriteLdlayer(writer, 0, layerStart);
    writer->Seek(layerStart, Engine::SeekOffsetType::Start);
...
static void WriteStereoSingleSeq(Engine::BinaryWriter* writer, uint16_t delay, uint8_t tempo, bool looped) {
    ...
    writer->Seek(placeHolder, Engine::SeekOffsetType::Start);
    ...
    writer->Seek(lLayerPlaceholderOff, Engine::SeekOffsetType::Start);
    WriteLdlayer(writer, 0, lLayerOffset);
    writer->Seek(rLayerPlaceholderOff, Engine::SeekOffsetType::Start);
    WriteLdlayer(writer, 1, rLayerOffset);
}

std::shared_ptr<Engine::IResource>
ResourceFactoryXMLAudioSequenceV0::ReadResource(std::shared_ptr<Engine::File> file,
                                                std::shared_ptr<Engine::ResourceInitData> initData) {
    if (!FileHasValidFormatAndReader(file, initData)) {
        return nullptr;
    }
    ...
    std::shared_ptr<Engine::File> seqFile;
    if (path != nullptr) {
        seqFile = Engine::Context::GetInstance()->GetResourceManager()->GetArchiveManager()->LoadFile(path);
    }
    ...
    if (!streamed) {
        ...
        if (path != nullptr) {
            ...
            Engine::BinaryWriter writer = Engine::BinaryWriter();
            writer.SetEndianness(Engine::Endianness::Big);
