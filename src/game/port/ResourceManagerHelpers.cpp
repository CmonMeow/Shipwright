...
extern "C" uint32_t ResourceMgr_GetGameVersion(int index) {
    return Engine::Context::GetInstance()->GetResourceManager()->GetArchiveManager()->GetGameVersions()[index];
}

extern "C" uint32_t ResourceMgr_GetGamePlatform(int index) {
    uint32_t version =
        Engine::Context::GetInstance()->GetResourceManager()->GetArchiveManager()->GetGameVersions()[index];
...
extern "C" bool ResourceMgr_IsAltAssetsEnabled() {
    return Engine::Context::GetInstance()->GetResourceManager()->IsAltAssetsEnabled();
}
...
std::shared_ptr<Engine::IResource> ResourceMgr_GetResourceByNameHandlingMQ(const char* path) {
    ...
    return Engine::Context::GetInstance()->GetResourceManager()->LoadResource(Path.c_str());
}
...
extern "C" SkeletonHeader* ResourceMgr_LoadSkeletonByName(const char* path, SkelAnime* skelAnime) {
    ...
    if (isAlt) {
        pathStr = Engine::IResource::gAltAssetPrefix + pathStr;
    }
