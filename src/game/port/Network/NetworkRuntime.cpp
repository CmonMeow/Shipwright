...
bool NetworkRuntime::SendPlayerState(NetworkPlayerStatePacket packet) {
...
bool NetworkRuntime::SendActorEvent(NetworkActorEventPacket packet) {
...
bool NetworkRuntime::SendProjectileState(NetworkProjectileStatePacket packet, bool reliableTransition) {
...
bool NetworkRuntime::SendProjectileImpact(NetworkProjectileImpactPacket packet) {
...
bool NetworkRuntime::SendVoice(NetworkVoicePacket packet) {
    if (!SaneVoice(packet)) {
        return false;
    }
    packet.playerId = LocalPlayerId();
...
bool NetworkRuntime::PollPlayerState(NetworkPlayerStatePacket& packet) {
...
bool NetworkRuntime::PollFishingState(NetworkPlayerStatePacket& packet) {
...
bool NetworkRuntime::PollPlayerRemove(NetworkPlayerRemovePacket& packet) {
...
bool NetworkRuntime::PollDynamicObjectState(NetworkDynamicObjectStatePacket& packet) {
...
bool NetworkRuntime::PollActorEvent(NetworkActorEventPacket& packet) {
...
bool NetworkRuntime::PollProjectileState(NetworkProjectileStatePacket& packet) {
...
bool NetworkRuntime::PollPlayerDamage(NetworkPlayerDamagePacket& packet) {
...
bool NetworkRuntime::PollPlayerRespawn(NetworkPlayerRespawnPacket& packet) {
...
void NetworkRuntime::OnClientMessage(char* buffer, __int32 size, void* context) {
    static_cast<NetworkRuntime*>(context)->HandleClientMessage(buffer, size);
}

void NetworkRuntime::OnServerMessage(__int32 sender, char* buffer, __int32 size, void* context) {
    static_cast<NetworkRuntime*>(context)->HandleServerMessage(sender, buffer, size);
}

void NetworkRuntime::OnPeerCreated(__int32 peer, bool, const char*, unsigned long, void* context) {
    static_cast<NetworkRuntime*>(context)->HandlePeerCreated(peer);
}

void NetworkRuntime::OnPeerDeleted(__int32 peer, void* context) {
    static_cast<NetworkRuntime*>(context)->HandlePeerDeleted(peer);
}

void NetworkRuntime::HandleClientMessage(char* buffer, __int32 size) {
    if (size > 0) {
...
void NetworkRuntime::HandleServerMessage(int32_t sender, char* buffer, __int32 size) {
    if (size > 0) {
        mInboundBytesSinceSample += static_cast<uint64_t>(size);
    }
    std::string decrypted;
...
bool NetworkRuntime::PrepareClientMessage(char* buffer, __int32 size, std::string& decrypted,
...
bool NetworkRuntime::PrepareServerMessage(int32_t sender, char* buffer, __int32 size,
...
bool NetworkRuntime::SendPlainToClient(NetAppMessageType type, const NetworkMessageRaw& raw) {
    return mClient && SendPayloadToServer(BuildAppRawMessage(type, raw), kReliable);
}

bool NetworkRuntime::SendPlainToPeer(int32_t peer, NetAppMessageType type, const NetworkMessageRaw& raw) {
    return mServer && SendPayloadToPeer(peer, BuildAppRawMessage(type, raw), kReliable);
}

bool NetworkRuntime::SendToServer(NetAppMessageType type, const NetworkMessageRaw& raw, NetMsgFlags flags) {
    return SendEncryptedPayloadToServer(BuildAppRawMessage(type, raw), flags);
}

bool NetworkRuntime::SendToPeer(int32_t peer, NetAppMessageType type, const NetworkMessageRaw& raw,
                                          NetMsgFlags flags) {
    return SendEncryptedPayloadToPeer(peer, BuildAppRawMessage(type, raw), flags);
}

bool NetworkRuntime::SendEncryptedPayloadToServer(const std::string& payload, NetMsgFlags flags) {
...
bool NetworkRuntime::SendEncryptedPayloadToPeer(int32_t peer, const std::string& payload,
                                                           NetMsgFlags flags) {
...
bool NetworkRuntime::SendPayloadToServer(const std::string& payload, NetMsgFlags flags) {
...
bool NetworkRuntime::SendPayloadToPeer(int32_t peer, const std::string& payload, NetMsgFlags flags) {
...
void NetworkRuntime::BeginClientCryptoHandshake() {
    std::string hello;
...
void NetworkRuntime::SendClientIdentity() {
    NetworkMessageRaw raw;
    EncodeLocalIdentityRaw(raw);
    mClientIdentitySent = SendToServer(NAMTConnect, raw, kReliable);
}

void NetworkRuntime::SendPrivateChatKeyFromClient() {
...
void NetworkRuntime::SendChatKeyTo(int32_t peer, int32_t owner, const std::string& name,
...
void NetworkRuntime::SendKnownChatKeysTo(int32_t peer) {
...
void NetworkRuntime::BroadcastChatKey(int32_t owner, const std::string& name,
                                                const std::string& publicKey) {
    for (const int32_t peer : mPeers) {
...
bool NetworkRuntime::EnsurePrivateChatKey() {
    if (mPrivateChatKeyReady) {
        return true;
    }
...
std::string NetworkRuntime::PrivateChatPublicKey() const {
    return std::string(reinterpret_cast<const char*>(mPrivateChatPublicKey), crypto_box_PUBLICKEYBYTES);
}

bool NetworkRuntime::EncryptPrivateText(int32_t target, const std::string& message,
...
bool NetworkRuntime::DecryptPrivateText(const std::string& cipher, std::string& message) const {
    message.clear();
...
std::string NetworkRuntime::PlayerName(int32_t player) const {
    if (player == 0) {
        return "system";
    }
...
bool NetworkRuntime::IsGameMaster(int32_t player) const {
    const auto identity = mIdentities.find(player);
...
bool NetworkRuntime::ResolvePlayerReference(const std::string& reference, int32_t& player) const {
...
void NetworkRuntime::SendCommandResult(int32_t player, const std::string& message) {
...
void NetworkRuntime::BroadcastSystem(const std::string& message) {
    const std::string line = "system: " + message;
    NetworkMessageRaw raw;
    raw.putString(line, CHAT_MAX_LINE_CHARS);
    Broadcast(NAMTChat, raw);
...
bool NetworkRuntime::KickPlayer(const std::string& reference, bool ban, std::string& result) {
    int32_t player = -1;
...
bool NetworkRuntime::GrantGameMaster(const std::string& reference, std::string& result) {
    int32_t player = -1;
    if (!mServer || !ResolvePlayerReference(reference, player)) {
        result = mServer ? "player not found" : "host only command";
        return false;
...
bool NetworkRuntime::RevokeGameMaster(const std::string& reference, std::string& result) {
...
bool NetworkRuntime::UnbanIdentity(const std::string& identity, std::string& result) {
...
void NetworkRuntime::SendUsersList(int32_t player) {
...
void NetworkRuntime::SendIdentityList(int32_t player, const char* label,
...
void NetworkRuntime::RunServerCommand(int32_t player, const std::string& command) {
...
void NetworkRuntime::QueueChat(const std::string& text, ChatLineKind kind) {
...
bool NetworkRuntime::DecodeChatKey(const char* message, __int32 size, int32_t& player,
...
bool NetworkRuntime::DecodePrivateForServer(const char* message, __int32 size, int32_t& target,
                                                      std::string& cipher) {
    NetworkMessageRaw raw;
...
bool NetworkRuntime::DecodePrivateForClient(const char* message, __int32 size, int32_t& sender,
...
bool NetworkRuntime::SanePlayerState(const NetworkPlayerStatePacket& packet) {
...
void NetworkRuntime::EvaluateMeleeAttack(int32_t player, const NetworkPlayerStatePacket& state) {
    if (!mServer) {
        return;
    }
...
void NetworkRuntime::SanitizeServerFishingState(
...
bool NetworkRuntime::AcceptServerActorEvent(int32_t player, NetworkActorEventPacket packet) {
...
void NetworkRuntime::ProcessPendingActorEvents() {
    const auto now = std::chrono::steady_clock::now();
    for (auto event = mPendingActorEvents.begin(); event != mPendingActorEvents.end();) {
...
void NetworkRuntime::UpdateServerDynamicObjects() {
    const auto now = std::chrono::steady_clock::now();
...
void NetworkRuntime::ReleaseFishOwnedBy(int32_t player) {
...
bool NetworkRuntime::SaneDynamicObjectState(const NetworkDynamicObjectStatePacket& packet) {
...
bool NetworkRuntime::SaneActorEvent(const NetworkActorEventPacket& packet) {
    NetworkDynamicObjectStatePacket objectState{ packet.sceneId, packet.roomId, packet.actorId,
                                                  packet.actorParams, packet.homeX, packet.homeY,
                                                  packet.homeZ, 0 };
...
void NetworkRuntime::ProcessServerDeathTransition(int32_t player,
...
void NetworkRuntime::UpdateServerRespawns() {
    const auto now = std::chrono::steady_clock::now();
...
void NetworkRuntime::CreateServerCorpse(const NetworkPlayerStatePacket& finalPose) {
    int32_t corpseId = 0;
...
bool NetworkRuntime::SaneProjectileState(const NetworkProjectileStatePacket& packet) {
    const auto saneFloat = [](float value) { return std::isfinite(value) && value > -1000000.0f && value < 1000000.0f; };
...
bool NetworkRuntime::SaneProjectileImpact(const NetworkProjectileImpactPacket& packet) {
    const auto saneFloat = [](float value) { return std::isfinite(value) && value > -1000000.0f && value < 1000000.0f; };
...
bool NetworkRuntime::AcceptServerProjectileImpact(int32_t witness,
...
bool NetworkRuntime::PlayerIsNearObject(
...
bool NetworkRuntime::AcceptServerProjectile(int32_t player,
...
void NetworkRuntime::RetainServerStuckArrow(
...
void NetworkRuntime::UpdateServerProjectiles() {
    const auto now = std::chrono::steady_clock::now();
    for (auto it = mServerProjectiles.begin(); it != mServerProjectiles.end();) {
...
bool NetworkRuntime::SaneVoice(const NetworkVoicePacket& packet) {
