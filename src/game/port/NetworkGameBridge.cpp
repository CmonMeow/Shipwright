...
extern "C" int32_t PCInput_IsBowAimHeld(void);
...
void QueueRemotePlayerNames(PlayState* play) {
    ...
    for (const auto& player : gNetworkGame.runtime->Players()) {
        ...
        Engine::Overlay::QueueCenteredGameText(player.name.c_str(), screenX, screenY,
...
extern "C" void NetworkGame_Initialize(int argc, char* argv[]) {
    ...
    gNetworkGame.multiplayerUI = std::make_unique<MultiplayerUI>();
    ...
    Engine::Overlay::SetMoveLoopCallback(NetworkGame_PumpMoveLoop);
}
