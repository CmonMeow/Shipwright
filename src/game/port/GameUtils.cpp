...
void GameUtils::RandInit(uint64_t seed, uint64_t* state) {
    if (state == nullptr) {
        state = &default_state;
    }
    *state = seed;
...
uint32_t GameUtils::next32(uint64_t* state) {
    if (state == nullptr) {
        state = &default_state;
        if (!default_init) {
...
uint32_t GameUtils::Random(uint32_t min, uint32_t max, uint64_t* state) {
    if (min == max) {
        return min;
    }
    assert(max > min);
...
double GameUtils::RandomDouble(uint64_t* state) {
