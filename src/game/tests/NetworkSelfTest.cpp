...
constexpr char kPing[] = "network-ping";
constexpr char kPong[] = "network-pong";
...
int main() {
    ...
    if (!server || !server->Init("Game transport test", "", kTestPort)) {
    ...
    if (!client || client->Init("127.0.0.1:47777", "", false, clientPort, "NetworkTest", nullptr) != CROK) {
