...
VecSph* OLib_Vec3fToVecSph(VecSph* dest, Vec3f* vec) {
    ...
    if ((dist == 0.0f) && (vec->y == 0.0f)) {
        ...
        sph.pitch = DEGF_TO_BINANG(RADF_TO_DEGF(atan2f(dist, vec->y)));
    }
    ...
    if ((vec->x == 0.0f) && (vec->z == 0.0f)) {
        ...
        sph.yaw = DEGF_TO_BINANG(RADF_TO_DEGF(atan2f(vec->x, vec->z)));
    }
...
Vec3f* OLib_Vec3fDiffRad(Vec3f* dest, Vec3f* a, Vec3f* b) {
    ...
    anglesRad.x = atan2f(b->z - a->z, b->y - a->y);
    anglesRad.y = atan2f(b->x - a->x, b->z - a->z);
    anglesRad.z = 0;
