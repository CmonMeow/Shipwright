...
extern void Overlay_QueueGameText(const char* text, float x, float y, float red, float green, float blue,
                                             float alpha);
...
static void GfxPrint_QueueSpan(GfxPrint* printer, const char* text, size_t length) {
    ...
    while (length != 0) {
        ...
        Overlay_QueueGameText(line, printer->posX * 0.25f, printer->posY * 0.25f,
