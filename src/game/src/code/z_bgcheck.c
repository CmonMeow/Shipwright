 ...
 */
int32_t BgCheck_PosErrorCheck(Vec3f* pos, char* file, int32_t line) {
 ...
 * Set SSNode
 */
void SSNode_SetValue(SSNode* node, int16_t* polyId, uint16_t next) {
    node->polyId = *polyId;
    node->next = next;
}
 ...
 */
void SSNodeList_SetSSListHead(SSNodeList* nodeList, SSList* ssList, int16_t* polyId) {
    uint16_t newNodeId = SSNodeList_GetNextNodeIdx(nodeList);
 ...
 */
void DynaSSNodeList_SetSSListHead(DynaSSNodeList* nodeList, SSList* ssList, int16_t* polyId) {
    uint16_t newNodeId = DynaSSNodeList_GetNextNodeIdx(nodeList);
 ...
 */
void DynaSSNodeList_Alloc(PlayState* play, DynaSSNodeList* nodeList, int32_t max) {
 ...
 */
uint16_t DynaSSNodeList_GetNextNodeIdx(DynaSSNodeList* nodeList) {
    uint16_t idx = nodeList->count++;
 ...
 */
int16_t CollisionPoly_GetMinY(CollisionPoly* poly, Vec3s* vtxList) {
    int32_t a = { 0 };
    int32_t b = { 0 };
    int32_t c = { 0 };
    int16_t min = { 0 };
 ...
 */
void CollisionPoly_GetNormalF(CollisionPoly* poly, float* nx, float* ny, float* nz) {
 ...
 */
int32_t CollisionPoly_CheckYIntersectApprox1(CollisionPoly* poly, Vec3s* vtxList, float x, float z, float* yIntersect,
                                         float chkDist) {
    static Vec3f polyVerts[3];

    Vec3s* vA = &vtxList[COLPOLY_VTX_INDEX(poly->flags_vIA)];
    Math_Vec3s_ToVec3f(&polyVerts[0], vA);
    Vec3s* vB = &vtxList[COLPOLY_VTX_INDEX(poly->flags_vIB)];
    Math_Vec3s_ToVec3f(&polyVerts[1], vB);
    Vec3s* vC = &vtxList[poly->vIC];
 ...
 */
int32_t CollisionPoly_CheckYIntersect(CollisionPoly* poly, Vec3s* vtxList, float x, float z, float* yIntersect, float chkDist) {
    static Vec3f polyVerts[3];
    float nx;
    float ny;
    float nz;
 ...
 * Determinant max 300.0f
 */
int32_t CollisionPoly_CheckYIntersectApprox2(CollisionPoly* poly, Vec3s* vtxList, float x, float z, float* yIntersect) {
 ...
 * Determinant max 300.0f
 */
int32_t CollisionPoly_CheckXIntersectApprox(CollisionPoly* poly, Vec3s* vtxList, float y, float z, float* xIntersect) {
    static Vec3f polyVerts[3];
    float nx;
    float ny;
    float nz;
 ...
 * Determinant max 300.0f
 */
int32_t CollisionPoly_CheckZIntersectApprox(CollisionPoly* poly, Vec3s* vtxList, float x, float y, float* zIntersect) {
    static Vec3f polyVerts[3];
    float nx;
    float ny;
    float nz;
 ...
 * normal's direction
 */
int32_t CollisionPoly_LineVsPoly(CollisionPoly* poly, Vec3s* vtxList, Vec3f* posA, Vec3f* posB, Vec3f* planeIntersect,
                             int32_t chkOneFace, float chkDist) {
    static Vec3f polyVerts[3];
    static Plane plane;
    float planeDistA;
    float planeDistB;
    float planeDistDelta = { 0 };
 ...
 */
int32_t CollisionPoly_SphVsPoly(CollisionPoly* poly, Vec3s* vtxList, Vec3f* center, float radius) {
 ...
 */
float BgCheck_RaycastFloorStaticList(CollisionContext* colCtx, uint16_t xpFlags, SSList* ssList, CollisionPoly** outPoly,
                                   Vec3f* pos, float yIntersectMin, float chkDist, int32_t flags) {
    SSNode* curNode = { 0 };
    float yIntersect;

    float result = yIntersectMin;
    if (ssList->head == SS_NULL) {
        return result;
    }
 ...
 * stores the pointer of the closest poly to `outPoly`
 */
float BgCheck_RaycastFloorStatic(StaticLookup* lookup, CollisionContext* colCtx, uint16_t xpFlags, CollisionPoly** poly,
                               Vec3f* pos, uint32_t arg5, float chkDist, float yIntersectMin) {
    int32_t flag = { 0 };
    float yIntersect = yIntersectMin;
 ...
 */
int32_t BgCheck_ComputeWallDisplacement(CollisionContext* colCtx, CollisionPoly* poly, float* posX, float* posZ, float nx, float ny,
                                    float nz, float invXZlength, float planeDist, float radius, CollisionPoly** wallPolyPtr) {
    uint32_t surfaceData = { 0 };
    uint32_t wallDamage = { 0 };
    float displacement = (radius - planeDist) * invXZlength;
 ...
 */
int32_t BgCheck_SphVsStaticWall(StaticLookup* lookup, CollisionContext* colCtx, uint16_t xpFlags, float* outX, float* outZ,
                            Vec3f* pos, float radius, CollisionPoly** outPoly) {
    Vec3f resultPos = { 0 };
    float planeDist = { 0 };
    float intersect;
    CollisionPoly* curPoly = { 0 };
    CollisionPoly* polyList = { 0 };
    SSNode* curNode = { 0 };
    float invNormalXZ = { 0 };
    int32_t polyId = { 0 };
    float normalXZ = { 0 };
    ...
    while (true) {
        ...
        float zTemp = vtxList[COLPOLY_VTX_INDEX(curPoly->flags_vIA)].z;
        float zMax = zMin = zTemp;
    ...
    while (true) {
        ...
        float xTemp = vtxList[COLPOLY_VTX_INDEX(curPoly->flags_vIA)].x;
...
int32_t BgCheck_CheckStaticCeiling(StaticLookup* lookup, uint16_t xpFlags, CollisionContext* colCtx, float* outY, Vec3f* pos,
    ...
    *outY = pos->y;
    ...
    while (true) {
        int32_t curPolyId = curNode->polyId;
        ...
        if (CollisionPoly_CheckYIntersectApprox2(curPoly, vtxList, pos->x, pos->z, &ceilingY)) {
            float intersectDist = ceilingY - *outY;
            float ny = COLPOLY_GET_NORMAL(curPoly->normal.y);
 ...
 */
int32_t BgCheck_CheckLineAgainstSSList(SSList* ssList, CollisionContext* colCtx, uint16_t xpFlags1, uint16_t xpFlags2, Vec3f* posA,
                                   Vec3f* posB, Vec3f* outPos, CollisionPoly** outPoly, float* outDistSq, float chkDist,
                                   int32_t bccFlags) {
    SSNode* curNode = { 0 };
    Vec3f polyIntersect;
    ...
    curNode = &colCtx->polyNodes.tbl[ssList->head];
    while (true) {
        int16_t polyId = curNode->polyId;
        uint8_t* checkedPoly = &colCtx->polyNodes.polyCheckTbl[polyId];
        ...
        *checkedPoly = true;
        CollisionPoly* curPoly = &polyList[polyId];
        float minY = CollisionPoly_GetMinY(curPoly, colCtx->colHeader->vtxList);
        ...
        if (CollisionPoly_LineVsPoly(curPoly, colCtx->colHeader->vtxList, posA, posB, &polyIntersect,
            ...
            float distSq = Math3D_Vec3fDistSq(posA, &polyIntersect);
            if (distSq < *outDistSq) {
 ...
 * `outDistSq` returns the squared distance from `posA` to the point of intersect
 */
int32_t BgCheck_CheckLineInSubdivision(StaticLookup* lookup, CollisionContext* colCtx, uint16_t xpFlags1, uint16_t xpFlags2,
                                   Vec3f* posA, Vec3f* posB, Vec3f* outPos, CollisionPoly** outPoly, float chkDist,
                                   float* outDistSq, uint32_t bccFlags) {
    int32_t result = false;
 ...
 */
int32_t BgCheck_SphVsFirstStaticPolyList(SSNode* node, uint16_t xpFlags, CollisionContext* colCtx, Vec3f* center, float radius,
                                     CollisionPoly** outPoly) {
    CollisionPoly* polyList = colCtx->colHeader->polyList;
    Vec3s* vtxList = colCtx->colHeader->vtxList;

    while (true) {
        int16_t curPolyId = node->polyId;
        CollisionPoly* curPoly = &polyList[curPolyId];
        if (COLPOLY_VIA_FLAG_TEST(colCtx->colHeader->polyList[curPolyId].flags_vIA, xpFlags)) {
 ...
 */
int32_t BgCheck_SphVsFirstStaticPoly(StaticLookup* lookup, uint16_t xpFlags, CollisionContext* colCtx, Vec3f* center, float radius,
                                 CollisionPoly** outPoly, uint16_t bciFlags) {
...
StaticLookup* BgCheck_GetNearestStaticLookup(CollisionContext* colCtx, StaticLookup* lookupTbl, Vec3f* pos) {
    ...
    int32_t subdivAmountX = colCtx->subdivAmount.x;
 ...
 */
void BgCheck_GetSubdivisionMinBounds(CollisionContext* colCtx, Vec3f* pos, int32_t* sx, int32_t* sy, int32_t* sz) {
    float dx = pos->x - colCtx->minBounds.x;
    float dy = pos->y - colCtx->minBounds.y;
    float dz = pos->z - colCtx->minBounds.z;

    *sx = dx * colCtx->subdivLengthInv.x;
    *sy = dy * colCtx->subdivLengthInv.y;
    *sz = dz * colCtx->subdivLengthInv.z;

    if (((int32_t)dx % (int32_t)colCtx->subdivLength.x < BGCHECK_SUBDIV_OVERLAP) && (*sx > 0)) {
        *sx -= 1;
    }

    if (((int32_t)dy % (int32_t)colCtx->subdivLength.y < BGCHECK_SUBDIV_OVERLAP) && (*sy > 0)) {
        *sy -= 1;
    }

    if (((int32_t)dz % (int32_t)colCtx->subdivLength.z < BGCHECK_SUBDIV_OVERLAP) && (*sz > 0)) {
        *sz -= 1;
    }
}
 ...
 * `sx`, `sy`, `sz` returns the subdivision x, y, z indices
 */
void BgCheck_GetSubdivisionMaxBounds(CollisionContext* colCtx, Vec3f* pos, int32_t* sx, int32_t* sy, int32_t* sz) {
    float dx = pos->x - colCtx->minBounds.x;
    float dy = pos->y - colCtx->minBounds.y;
    float dz = pos->z - colCtx->minBounds.z;
    ...
    *sz = dz * colCtx->subdivLengthInv.z;
    ...
    if (((int32_t)colCtx->subdivLength.x - BGCHECK_SUBDIV_OVERLAP < (int32_t)dx % (int32_t)colCtx->subdivLength.x) &&
        (*sx < colCtx->subdivAmount.x - 1)) {
        *sx += 1;
    }

    if (((int32_t)colCtx->subdivLength.y - BGCHECK_SUBDIV_OVERLAP < (int32_t)dy % (int32_t)colCtx->subdivLength.y) &&
        (*sy < colCtx->subdivAmount.y - 1)) {
        *sy += 1;
    }

    if (((int32_t)colCtx->subdivLength.z - BGCHECK_SUBDIV_OVERLAP < (int32_t)dz % (int32_t)colCtx->subdivLength.z) &&
...
void BgCheck_GetPolySubdivisionBounds(CollisionContext* colCtx, Vec3s* vtxList, CollisionPoly* polyList,
                                      int32_t* subdivMinX, int32_t* subdivMinY, int32_t* subdivMinZ, int32_t* subdivMaxX,
                                      int32_t* subdivMaxY, int32_t* subdivMaxZ, int16_t polyId) {
    uint16_t* vtxDataTemp;
    Vec3f minVtx;
    Vec3f maxVtx;


    int16_t vtxId = COLPOLY_VTX_INDEX(polyList[polyId].vtxData[0]);
 ...
 */
int32_t BgCheck_PolyIntersectsSubdivision(Vec3f* min, Vec3f* max, CollisionPoly* polyList, Vec3s* vtxList, int16_t polyId) {
    float intersect;
    Vec3f va2;
    Vec3f vb2;
    Vec3f vc2;
 ...
 */
uint32_t BgCheck_InitializeStaticLookup(CollisionContext* colCtx, PlayState* play, StaticLookup* lookupTbl) {
    Vec3s* vtxList = { 0 };
    CollisionPoly* polyList = { 0 };
    int32_t polyMax = { 0 };
    int32_t polyIdx;
    ...
    StaticLookup* spA4;
    int32_t sp98 = { 0 };
    float subdivLengthX = { 0 };
    float subdivLengthY = { 0 };
    float subdivLengthZ = { 0 };
    ...
    for (polyIdx = 0; polyIdx < polyMax; polyIdx++) {
        ...
        for (sz = szMin; sz < szMax + 1; sz++) {
            StaticLookup* phi_fp = (colCtx->subdivAmount.x * syMin) + spA4;
            ...
            for (sy = syMin; sy < syMax + 1; sy++) {
                StaticLookup* phi_s0 = sxMin + phi_fp;
 ...
 */
int32_t BgCheck_IsSpotScene(PlayState* play) {
    static int16_t spotScenes[] = {
 ...
 */
int32_t BgCheck_TryGetCustomMemsize(int32_t sceneId, uint32_t* memSize) {
 ...
 */
void BgCheck_SetSubdivisionDimension(float min, int32_t subdivAmount, float* max, float* subdivLength, float* subdivLengthInv) {
    float length = (*max - min);

    *subdivLength = (int32_t)(length / subdivAmount) + 1;
...
typedef struct {
    int16_t sceneId;
    Vec3s subdivAmount;
    int32_t nodeListMax; // if -1, dynamically compute max nodes
...
void BgCheck_Allocate(CollisionContext* colCtx, PlayState* play, CollisionHeader* colHeader) {
    ...
    uint32_t tblMax = { 0 };
    uint32_t memSize;
    uint32_t lookupTblMemSize = { 0 };
    uint32_t customMemSize;
    int32_t i;

    colCtx->colHeader = colHeader;
    int32_t customNodeListMax = -1;
    ...
    if (YREG(15) == 0x10 || YREG(15) == 0x20 || YREG(15) == 0x30 || YREG(15) == 0x40) {
        ...
        colCtx->dyna.polyListMax = 512;
        colCtx->dyna.vtxListMax = 512;
        int32_t useCustomSubdivisions = false;
    ...
    memSize = colCtx->subdivAmount.x * sizeof(StaticLookup) * colCtx->subdivAmount.y * colCtx->subdivAmount.z +
              colCtx->colHeader->numPolygons * sizeof(uint8_t) + colCtx->dyna.polyNodesMax * sizeof(SSNode) +
 ...
 */
float BgCheck_RaycastFloorImpl(PlayState* play, CollisionContext* colCtx, uint16_t xpFlags, CollisionPoly** outPoly,
                             int32_t* outBgId, Vec3f* pos, Actor* actor, uint32_t arg7, float chkDist) {

    float yIntersectDyna = { 0 };
    DynaRaycast dynaRaycast;

    *outBgId = BGCHECK_SCENE;
    *outPoly = NULL;
    StaticLookup* lookupTbl = colCtx->lookupTbl;
    float yIntersect = BGCHECK_Y_MIN;
    Vec3f checkPos = *pos;
    ...
    while (true) {
        ...
        StaticLookup* lookup = BgCheck_GetStaticLookup(colCtx, lookupTbl, &checkPos);
        if (lookup == NULL) {
 ...
 */
float BgCheck_CameraRaycastFloor1(CollisionContext* colCtx, CollisionPoly** outPoly, Vec3f* pos) {
    int32_t bgId;
 ...
 * returns yIntersect of the poly found, or BGCHECK_Y_MIN if no poly detected
 */
float BgCheck_EntityRaycastFloor2(PlayState* play, CollisionContext* colCtx, CollisionPoly** outPoly, Vec3f* pos) {
    int32_t bgId;
 ...
 * returns yIntersect of the poly found, or BGCHECK_Y_MIN if no poly detected
 */
float BgCheck_EntityRaycastFloor7(CollisionContext* colCtx, CollisionPoly** outPoly, int32_t* bgId, Actor* actor,
                                Vec3f* pos) {
...
float BgCheck_AnyRaycastFloor1(CollisionContext* colCtx, CollisionPoly* outPoly, Vec3f* pos) {
    ...
    float result = BgCheck_RaycastFloorImpl(NULL, colCtx, COLPOLY_IGNORE_NONE, &tempPoly, &bgId, pos, NULL, 0x1C, 1.0f);
 ...
 * returns yIntersect of the poly found, or BGCHECK_Y_MIN if no poly detected
 */
float BgCheck_EntityRaycastFloor8(CollisionContext* colCtx, CollisionPoly** outPoly, int32_t* bgId, Actor* actor,
                                Vec3f* pos) {
...
int32_t BgCheck_CheckWallImpl(CollisionContext* colCtx, uint16_t xpFlags, Vec3f* posResult, Vec3f* posNext, Vec3f* posPrev,
    ...
    CollisionPoly* poly;
    float dx, dy, dz; // change between posPrev to posNext
    Vec3f sphCenter = { 0 };
    int32_t dynaPolyCollision = { 0 };
    Vec3f posIntersect;
    int32_t bgId;
    float f32temp = { 0 };
    float nx2, nz2;
    ...
    Vec3f posIntersect2;
    int32_t bgId2;
    float nx, ny, nz; // unit normal of polygon
    ...
    *posResult = *posNext;
    ...
    if ((dx != 0.0f || dz != 0.0f) && (argA & 1) == 0) {
        if ((checkHeight + dy) < 5.0f) {
            ...
            if (result) {
                ...
                float n2XZDist = sqrtf(SQ(nx2) + SQ(nz2));
    ...
    if (dynaPolyCollision == true || *outBgId != BGCHECK_SCENE) {
        if (BgCheck_CheckLineImpl(colCtx, xpFlags, COLPOLY_IGNORE_NONE, posPrev, posResult, &posIntersect2, &poly,
            ...
            float n3XZDist = sqrtf(SQ(nx3) + SQ(nz3));

            // if poly is not a "flat" floor or "flat" ceiling
            if (!IS_ZERO(n3XZDist)) {
                // normalize nx,nz and multiply each by the radius to go back to the other side of the wall
                f32temp = 1.0f / n3XZDist;
                float temp_f0_2 = radius * f32temp;
 ...
 */
int32_t BgCheck_CheckCeilingImpl(CollisionContext* colCtx, uint16_t xpFlags, float* outY, Vec3f* pos, float checkHeight,
                             CollisionPoly** outPoly, int32_t* outBgId, Actor* actor) {
    StaticLookup* lookupTbl = { 0 };
    StaticLookup* lookup = { 0 };
    int32_t result = { 0 };
    Vec3f posTemp = { 0 };
    float tempY = { 0 };
 ...
 * `outY` returns the displaced y coordinate needed to not collide with the poly
 */
int32_t BgCheck_EntityCheckCeiling(CollisionContext* colCtx, float* outY, Vec3f* pos, float checkHeight,
                               CollisionPoly** outPoly, int32_t* outBgId, Actor* actor) {
...
int32_t BgCheck_CheckLineImpl(CollisionContext* colCtx, uint16_t xpFlags1, uint16_t xpFlags2, Vec3f* posA, Vec3f* posB,
                          Vec3f* posResult, CollisionPoly** outPoly, int32_t* outBgId, Actor* actor, float chkDist,
                          uint32_t bccFlags) {
    StaticLookup* lookupTbl = colCtx->lookupTbl;
    int32_t subdivMin[3];
    int32_t subdivMax[3];
    ...
    *outPoly = NULL;
    ...
    if (subdivMin[0] != subdivMax[0] || subdivMin[1] != subdivMax[1] || subdivMin[2] != subdivMax[2]) {
        ...
        int32_t temp_lo = colCtx->subdivAmount.x * colCtx->subdivAmount.y;
        StaticLookup* iLookup = lookupTbl + subdivMin[2] * temp_lo;
 ...
 * Get bccFlags
 */
uint32_t BgCheck_GetBccFlags(int32_t chkWall, int32_t chkFloor, int32_t chkCeil, int32_t chkOneFace, int32_t chkDyna) {
    uint32_t result = 0;
 ...
 */
int32_t BgCheck_SphVsFirstPolyImpl(CollisionContext* colCtx, uint16_t xpFlags, CollisionPoly** outPoly, int32_t* outBgId,
 ...
 */
int32_t BgCheck_SphVsFirstWall(CollisionContext* colCtx, Vec3f* center, float radius) {
    CollisionPoly* poly;
    int32_t bgId;
 ...
 */
int32_t ScaleRotPos_Equals(ScaleRotPos* a, ScaleRotPos* b) {
 ...
 * Reset vtxStartIndex
 */
void DynaLookup_ResetVtxStartIndex(uint16_t* vtxStartIndex) {
    *vtxStartIndex = 0;
}
 ...
 */
int32_t BgActor_IsTransformUnchanged(BgActor* bgActor) {
 ...
 */
void DynaPoly_AllocPolyList(PlayState* play, CollisionPoly** polyList, int32_t numPolys) {
 ...
 */
void DynaPoly_AllocVtxList(PlayState* play, Vec3s** vtxList, int32_t numVtx) {
...
void func_8003ED00(PlayState* play, DynaCollisionContext* dyna, int32_t bgId) {
    if (DynaPoly_IsBgIdBgActor(bgId)) {
        dyna->bgActorFlags[bgId] &= ~8;
        dyna->bitFlag |= DYNAPOLY_INVALIDATE_LOOKUP;
    }
 ...
 */
void DynaPoly_ExpandSRT(PlayState* play, DynaCollisionContext* dyna, int32_t bgId, int32_t* vtxStartIndex,
                        int32_t* polyStartIndex) {
    MtxF mtx;
    int32_t i;
    ...
    CollisionHeader* pbgdata = dyna->bgActors[bgId].colHeader;
    Sphere16* sphere = &dyna->bgActors[bgId].boundingSphere;
    Actor* actor = dyna->bgActors[bgId].actor;
    ...
    if (!(dyna->bitFlag & DYNAPOLY_INVALIDATE_LOOKUP) &&
        ...
        for (pi = *polyStartIndex; pi < *polyStartIndex + pbgdata->numPolygons; pi++) {
            ...
            int16_t normalY = poly->normal.y;

            if (normalY > COLPOLY_SNORMAL(0.5f)) {
                int16_t polyIndex = pi;
        ...
        *vtxStartIndex += pbgdata->numVertices;
        ...
        float numVtxInverse = 1.0f / pbgdata->numVertices;
        ...
        for (i = 0; i < pbgdata->numVertices; i++) {
            ...
            float radiusSq = Math3D_Vec3fDistSq(&newVtx, &newCenterPoint);
        ...
        for (i = 0; i < pbgdata->numPolygons; i++) {
            ...
            *newPoly = pbgdata->polyList[i];
            ...
            float newNormMagnitude = Math3D_Vec3fMagnitude(&newNormal);
            ...
            if (newNormal.y > 0.5f) {
                int16_t polyId = *polyStartIndex + i;
...
void DynaPoly_Setup(PlayState* play, DynaCollisionContext* dyna) {
    DynaPolyActor* actor;
    int32_t vtxStartIndex = { 0 };
    int32_t polyStartIndex = { 0 };
    int32_t i;
 ...
 */
float BgCheck_RaycastFloorDynaList(DynaRaycast* dynaRaycast, uint32_t listType) {
    CollisionPoly* polyList = { 0 };
    SSNode* curNode = { 0 };
    float yIntersect;

    float result = dynaRaycast->yIntersect;
    ...
    *dynaRaycast->bgId = BGCHECK_SCENE;
    ...
    if ((result != BGCHECK_Y_MIN) && (dynaActor != NULL) && (dynaRaycast->play != NULL)) {
        if (dynaRaycast->colCtx->dyna.bgActorFlags[*dynaRaycast->bgId] & 2) {
            ScaleRotPos* curTransform = &dynaRaycast->dyna->bgActors[*dynaRaycast->bgId].curTransform;
            polyMin =
                &dynaRaycast->dyna->polyList[dynaRaycast->dyna->bgActors[*dynaRaycast->bgId].dynaLookup.polyStartIndex];
            uint32_t polyIndex = *dynaRaycast->resultPoly - polyMin;
            CollisionPoly* poly = &dynaRaycast->dyna->bgActors[*dynaRaycast->bgId].colHeader->polyList[polyIndex];
            ...
            Vec3s* vtxList = dynaRaycast->dyna->bgActors[*dynaRaycast->bgId].colHeader->vtxList;
 ...
 */
int32_t BgCheck_SphVsDynaWallInBgActor(CollisionContext* colCtx, uint16_t xpFlags, DynaCollisionContext* dyna, SSList* ssList,
                                   float* outX, float* outZ, CollisionPoly** outPoly, int32_t* outBgId, Vec3f* pos, float radius,
                                   int32_t bgId) {
    float intersect;
    int32_t result = false;
    CollisionPoly* poly = { 0 };
    SSNode* curNode = { 0 };
    float nx;
    float ny;
    float nz;
    ...
    while (true) {
        ...
        float zTemp = dyna->vtxList[COLPOLY_VTX_INDEX(poly->flags_vIA)].z;
        float zMax = zMin = zTemp;
    ...
    while (true) {
        ...
        float xTemp = dyna->vtxList[COLPOLY_VTX_INDEX(poly->flags_vIA)].x;
        float xMax = xMin = xTemp;
        ...
        if (CollisionPoly_CheckXIntersectApprox(poly, dyna->vtxList, pos->y, resultPos.z, &intersect)) {
            float xIntersectDist = intersect - resultPos.x;
...
int32_t BgCheck_SphVsDynaWall(CollisionContext* colCtx, uint16_t xpFlags, float* outX, float* outZ, Vec3f* pos, float radius,
    ...
    for (i = 0; i < BG_ACTOR_MAX; i++) {
        ...
        bgActor->boundingSphere.radius += (int16_t)radius;

        float r = bgActor->boundingSphere.radius;
        float dx = bgActor->boundingSphere.center.x - resultPos.x;
        float dz = bgActor->boundingSphere.center.z - resultPos.z;
 ...
 * `outY` returns the y coordinate needed to not collide with `outPoly`
 */
int32_t BgCheck_CheckDynaCeilingList(CollisionContext* colCtx, uint16_t xpFlags, DynaCollisionContext* dyna, SSList* ssList,
                                 float* outY, Vec3f* pos, float checkHeight, CollisionPoly** outPoly) {
    SSNode* curNode = { 0 };
    Vec3f testPos = { 0 };
    float ceilingY;
    float nx;
    ...
    while (true) {
        int16_t polyId = curNode->polyId;
        CollisionPoly* poly = &dyna->polyList[polyId];
 ...
 */
int32_t BgCheck_CheckLineAgainstBgActorSSList(DynaLineTest* dynaLineTest) {
    int32_t result = { 0 };
    SSNode* curNode = { 0 };
    Vec3f polyIntersect;
    ...
    while (true) {
        int16_t polyId = curNode->polyId;
        CollisionPoly* curPoly = &dynaLineTest->dyna->polyList[polyId];
        if (COLPOLY_VIA_FLAG_TEST(curPoly->flags_vIA, dynaLineTest->xpFlags)) {
            ...
            if (CollisionPoly_LineVsPoly(curPoly, dynaLineTest->dyna->vtxList, dynaLineTest->posA, dynaLineTest->posB,
                ...
                float distSq = Math3D_Vec3fDistSq(dynaLineTest->posA, &polyIntersect);
 ...
 */
int32_t BgCheck_CheckLineAgainstBgActor(CollisionContext* colCtx, uint16_t xpFlags, Vec3f* posA, Vec3f* posB, Vec3f* posResult,
                                    CollisionPoly** outPoly, float* distSq, int32_t bgId, float chkDist, int32_t bccFlags) {
    int32_t result = false;
 ...
 * `outPoly` returns the pointer of the first poly found that intersects
 */
int32_t BgCheck_SphVsFirstDynaPolyList(CollisionContext* colCtx, uint16_t xpFlags, CollisionPoly** outPoly, Vec3f* center,
                                   float radius, SSList* ssList) {
    DynaCollisionContext* dyna = { 0 };
    SSNode* curNode = { 0 };
    ...
    while (true) {
        int32_t curPolyId = curNode->polyId;
        CollisionPoly* curPoly = &dyna->polyList[curPolyId];
 ...
 * `outPoly` returns the pointer of the first poly found that intersects
 */
int32_t BgCheck_SphVsFirstDynaPolyInBgActor(CollisionContext* colCtx, uint16_t xpFlags, CollisionPoly** outPoly, Vec3f* center,
                                        float radius, int32_t bgId, uint16_t bciFlags) {
...
void func_800418D0(CollisionContext* colCtx, PlayState* play) {
    ...
    for (i = 0; i < BG_ACTOR_MAX; i++) {
        uint16_t flag = dyna->bgActorFlags[i];
        if ((flag & 1) && !(flag & 2)) {
 ...
 */
uint32_t SurfaceType_GetData(CollisionContext* colCtx, CollisionPoly* poly, int32_t bgId, int32_t dataIdx) {
 ...
 */
uint16_t func_80041B24(CollisionContext* colCtx, uint32_t camId, int32_t bgId) {
    CollisionHeader* colHeader = BgCheck_GetCollisionHeader(colCtx, bgId);
    CamData* camData = { 0 };
...
uint16_t SurfaceType_GetSfx(CollisionContext* colCtx, CollisionPoly* poly, int32_t bgId) {
    int32_t id = func_80041F10(colCtx, poly, bgId);
 ...
 */
float zdWaterBoxMinX = -348.0f;
float zdWaterBoxMinY = 777.0f;
float zdWaterBoxMinZ = -1746.0f;
float zdWaterBoxMaxX = 205.0f;
float zdWaterBoxMaxY = 977.0f;
float zdWaterBoxMaxZ = -967.0f;
...
int32_t WaterBox_GetSurfaceImpl(PlayState* play, CollisionContext* colCtx, float x, float z, float* ySurface,
    ...
    for (curWaterBox = colHeader->waterBoxes; curWaterBox < colHeader->waterBoxes + colHeader->numWaterBoxes;
         curWaterBox++) {
        uint32_t room = (curWaterBox->properties >> 13) & 0x3F;
        if (room == (uint32_t)play->roomCtx.curRoom.num || room == 0x3F) {
...
uint32_t WaterBox_GetCamDataIndex(CollisionContext* colCtx, WaterBox* waterBox) {
    uint32_t prop = waterBox->properties >> 0;
...
uint16_t WaterBox_GetCameraSType(CollisionContext* colCtx, WaterBox* waterBox) {
    int32_t camId = WaterBox_GetCamDataIndex(colCtx, waterBox);
...
uint32_t WaterBox_GetLightSettingIndex(CollisionContext* colCtx, WaterBox* waterBox) {
    uint32_t prop = waterBox->properties >> 8;
 ...
 * `ySurface` returns the water box's surface, while `outWaterBox` returns a pointer to the WaterBox
 */
int32_t func_800425B0(PlayState* play, CollisionContext* colCtx, float x, float z, float* ySurface, WaterBox** outWaterBox) {
    CollisionHeader* colHeader = colCtx->colHeader;
    WaterBox* curWaterBox;
    ...
    for (curWaterBox = colHeader->waterBoxes; curWaterBox < colHeader->waterBoxes + colHeader->numWaterBoxes;
         curWaterBox++) {
        uint32_t room = (curWaterBox->properties >> 0xD) & 0x3F;
        if ((room == (uint32_t)play->roomCtx.curRoom.num) || (room == 0x3F)) {
            if ((curWaterBox->properties & 0x80000) != 0) {
                if (curWaterBox->xMin < x && x < (curWaterBox->xMin + curWaterBox->xLength)) {
                    if (curWaterBox->zMin < z && z < (curWaterBox->zMin + curWaterBox->zLength)) {
...
void BgCheck_DrawDynaPolyList(PlayState* play, CollisionContext* colCtx, DynaCollisionContext* dyna, SSList* ssList,
    ...
    if (ssList->head != SS_NULL) {
        SSNode* curNode = &dyna->polyNodes.tbl[ssList->head];
        while (true) {
            int16_t curPolyId = curNode->polyId;
            CollisionPoly* poly = &dyna->polyList[curPolyId];
            ...
            BgCheck_Vec3sToVec3f((int32_t)(poly->vIC) + dyna->vtxList, &vC);
            if (AREG(26)) {
                float nx = COLPOLY_GET_NORMAL(poly->normal.x);
                float ny = COLPOLY_GET_NORMAL(poly->normal.y);
                float nz = COLPOLY_GET_NORMAL(poly->normal.z);
 ...
 */
void BgCheck_DrawBgActor(PlayState* play, CollisionContext* colCtx, int32_t bgId) {
    if (AREG(21)) {
 ...
 * Draw a static poly
 */
void BgCheck_DrawStaticPoly(PlayState* play, CollisionContext* colCtx, CollisionPoly* poly, uint8_t r, uint8_t g, uint8_t b) {
    Vec3f vA;
    Vec3f vB;
    Vec3f vC;
 ...
 */
void BgCheck_DrawStaticPolyList(PlayState* play, CollisionContext* colCtx, SSList* ssList, uint8_t r, uint8_t g, uint8_t b) {
    CollisionPoly* polyList = colCtx->colHeader->polyList;
