#include <runtime/log/Log.hpp>
#include <engine/resource/ResourceManager.h>
#include "platform/client/RetainedGameBridge.h"
#include "resources/SceneLoader.h"
#include "resources/type/Scene.h"
#include "global.h"
#include "vt.h"

extern "C" void Play_InitEnvironment(PlayState* play, int16_t skyboxId);
static void InitializeScene(PlayState* play, int32_t spawn);

static Engine::IResource* LoadSceneAsset(const char* fileName) {
    auto res = RetainedGame_GetResourceManager()->LoadResource(fileName);
    return res.get();
}

extern "C" void SceneLoader_SpawnScene(PlayState* play, int32_t sceneId, int32_t spawn) {
    SceneTableEntry* scene = &gSceneTable[sceneId];

    scene->unk_13 = 0;
    play->loadedScene = scene;
    play->sceneNum = sceneId;
    play->sceneConfig = scene->config;

    // osSyncPrintf("\nSCENE SIZE %fK\n", (scene->sceneFile.vromEnd - scene->sceneFile.vromStart) / 1024.0f);

    const std::string scenePath = "scenes/shared/test01_scene/test01_scene";

    play->sceneSegment = LoadSceneAsset(scenePath.c_str());

    // The reduced runtime has no fallback scene.
    if (play->sceneSegment == nullptr) {
        WriteLog("Unable to load required test01 scene: {}", scenePath);
        play->state.running = false;
        return;
    }

    scene->unk_13 = 0;

    // gSegments[2] = VIRTUAL_TO_PHYSICAL(play->sceneSegment);

    InitializeScene(play, spawn);
    auto roomSize = func_80096FE8(play, &play->roomCtx);

    osSyncPrintf("ROOM SIZE=%fK\n", roomSize / 1024.0f);

    WriteLog("Scene Init - sceneNum: {0:#x}, entranceIndex: {1:#x}", play->sceneNum, gSaveContext.entranceIndex);
}

static void InitializeScene(PlayState* play, int32_t spawn) {
    play->curSpawn = spawn;
    play->linkActorEntry = nullptr;
    play->unk_11DFC = nullptr;
    play->setupEntranceList = nullptr;
    Object_InitBank(play, &play->objectCtx);
    LightContext_Init(play, &play->lightCtx);
    func_80096FD4(play, &play->roomCtx.curRoom);
    YREG(15) = 0;
    gSaveContext.worldMapArea = 0;
    Game::Resources::ExecuteSceneCommands(play, static_cast<Game::Resources::Scene*>(play->sceneSegment));

    Play_InitEnvironment(play, play->skyboxId);
}
