...
#define G_SETTIMG_RESOURCE_HASH 0x20
#define G_SETFB 0x21
#define G_RESETFB 0x22
#define G_SETTIMG_FB 0x23
#define G_VTX_RESOURCE_PATH 0x24
#define G_SETTIMG_RESOURCE_PATH 0x25
#define G_TRI1_RESOURCE 0x26
#define G_DL_RESOURCE_PATH 0x27
#define G_PUSHCD 0x28
#define G_MTX_RESOURCE_PATH 0x29
#define G_DL_RESOURCE_HASH 0x31
#define G_VTX_RESOURCE_HASH 0x32
#define G_MARKER 0x33
#define G_INVALTEXCACHE 0x34
#define G_BRANCH_Z_RESOURCE 0x35
#define G_MTX_RESOURCE 0x36
#define G_TEXRECT_WIDE 0x37
#define G_FILLWIDERECT 0x38
#define G_REGBLENDEDTEX 0x3f
#define G_MOVEMEM_RESOURCE 0x42
...
#define gSPExtraGeometryMode(pkt, c, s)                                                 \
    _DW({                                                                               \
        Gfx* _g = (Gfx*)(pkt);                                                          \
                                                                                        \
        _g->words.w0 = _SHIFTL(G_EXTRAGEOMETRYMODE, 24, 8) | _SHIFTL(~(uint32_t)(c), 0, 24); \
...
#define gSPGeometryMode(pkt, c, s)                                                 \
    _DW({                                                                          \
        Gfx* _g = (Gfx*)(pkt);                                                     \
        _g->words.w0 = _SHIFTL(G_GEOMETRYMODE, 24, 8) | _SHIFTL(~(uint32_t)(c), 0, 24); \
        _g->words.w1 = (uint32_t)(s);                                                   \
    })

#define gsSPGeometryMode(c, s) \
    { (_SHIFTL(G_GEOMETRYMODE, 24, 8) | _SHIFTL(~(uint32_t)(c), 0, 24)), (uint32_t)(s) }
...
#define gSPScisTextureRectangle(pkt, xl, yl, xh, yh, tile, s, t, dsdx, dtdy)                                       \
    _DW({                                                                                                          \
        Gfx* _g = (Gfx*)(pkt);                                                                                     \
                                                                                                                   \
        _g->words.w0 =                                                                                             \
            (_SHIFTL(G_TEXRECT, 24, 8) | _SHIFTL(MAX((int16_t)(xh), 0), 12, 12) | _SHIFTL(MAX((int16_t)(yh), 0), 0, 12));  \
        _g->words.w1 =                                                                                             \
            (_SHIFTL((tile), 24, 3) | _SHIFTL(MAX((int16_t)(xl), 0), 12, 12) | _SHIFTL(MAX((int16_t)(yl), 0), 0, 12));     \
        gImmp1(pkt, G_RDPHALF_1,                                                                                   \
               (_SHIFTL(((s) - (((int16_t)(xl) < 0) ? (((int16_t)(dsdx) < 0) ? (MAX((((int16_t)(xl) * (int16_t)(dsdx)) >> 7), 0))  \
                                                                     : (MIN((((int16_t)(xl) * (int16_t)(dsdx)) >> 7), 0))) \
                                                : 0)),                                                             \
                        16, 16) |                                                                                  \
                _SHIFTL(((t) - (((yl) < 0) ? (((int16_t)(dtdy) < 0) ? (MAX((((int16_t)(yl) * (int16_t)(dtdy)) >> 7), 0))       \
                                                                : (MIN((((int16_t)(yl) * (int16_t)(dtdy)) >> 7), 0)))      \
                                           : 0)),                                                                  \
                        0, 16)));                                                                                  \
        gImmp1(pkt, G_RDPHALF_2, (_SHIFTL((dsdx), 16, 16) | _SHIFTL((dtdy), 0, 16)));                              \
    })
