...
AudioPlayer::~AudioPlayer() {
    WriteLog("destruct audio player");
}
...
bool AudioPlayer::Init() {
    ...
    if (mAudioSettings.ChannelSetting == AudioChannelsSetting::audioMatrix51) {
        WriteLog("Initializing sound matrix decoder for surround");
...
bool AudioPlayer::SetAudioChannels(AudioChannelsSetting channels) {
    ...
    WriteLog("Changing audio channels from {} to {}", AudioChannelsSettingName(mAudioSettings.ChannelSetting),
...
void AudioPlayer::Play(const uint8_t* buf, size_t len) {
    ...
    if (!mSoundMatrixDecoder) {
        WriteLog("AudioPlayer: Matrix 5.1 mode enabled but SoundMatrixDecoder is not initialized");
        return;
    }
