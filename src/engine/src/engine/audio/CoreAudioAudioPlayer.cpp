...
CoreAudioAudioPlayer::~CoreAudioAudioPlayer() {
    WriteLog("destruct CoreAudio audio player");
    DoClose();
...
bool CoreAudioAudioPlayer::DoInit() {
    ...
    if (component == NULL) {
        WriteLog("CoreAudio: Failed to find audio component");
        return false;
    }

    status = AudioComponentInstanceNew(component, &mAudioUnit);
    if (status != noErr) {
        WriteLog("CoreAudio: Failed to create audio component instance: {}", status);
        return false;
    }
    ...
    status = AudioUnitSetProperty(mAudioUnit, kAudioOutputUnitProperty_EnableIO, kAudioUnitScope_Output, 0, &flag,
                                  sizeof(flag));
    if (status != noErr) {
        WriteLog("CoreAudio: Failed to enable output: {}", status);
        return false;
    }
    ...
    status = AudioUnitSetProperty(mAudioUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Input, 0, &format,
                                  sizeof(format));
    if (status != noErr) {
        WriteLog("CoreAudio: Failed to set stream format: {}", status);
        return false;
    }
    ...
    if (status != noErr) {
        WriteLog("CoreAudio: Failed to set render callback: {}", status);
        return false;
    }

    status = AudioUnitInitialize(mAudioUnit);
    if (status != noErr) {
        WriteLog("CoreAudio: Failed to initialize audio unit: {}", status);
        return false;
    }

    status = AudioOutputUnitStart(mAudioUnit);
    if (status != noErr) {
        WriteLog("CoreAudio: Failed to start audio unit: {}", status);
        return false;
    }
