...
bool WasapiAudioPlayer::DoInit() {
    try {
        if (!mComInitialized) {
            ...
            mComNeedsUninitialize = SUCCEEDED(initResult);
        }
        ...
        WriteLog("WASAPI initialization failed: 0x{:08X}", static_cast<unsigned long>(res));
        return false;
    }
