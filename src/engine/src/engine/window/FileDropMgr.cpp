...
void FileDropMgr::SetDroppedFile(char* path) {
    if (mPath != nullptr) {
        WriteLog("Overwriting dropped file: {} with {}", mPath, path);
        free(mPath);
    }
    mPath = strdup(path);
    mFileDropped = true;
...
static void PrintRegError(void* funcAddr) {
    ...
    WriteLog("Trying to register {}. Already registered.", nameFound);
    ...
    WriteLog("Trying to register {}. Already registered.", symbol->Name);
    SymCleanup(hProcess);
#endif
}
...
void FileDropMgr::CallHandlers() {
    ...
    WriteLog("Dropped file {} not handled by any registered.", mPath);
}
