...
ArchiveManager::~ArchiveManager() {
    WriteLog("destruct archive manager");
    SetArchives(nullptr);
}
    ...
    for (const auto& archivePath : archivePaths) {
        if (archivePath.length() > 0) {
            if (std::filesystem::is_directory(archivePath)) {
                ...
                WriteLog("The archive at path {} does not exist", std::filesystem::absolute(archivePath).string());
            }
        } else {
            WriteLog("No archive path supplied");
        }
    }
...
std::shared_ptr<Archive> ArchiveManager::AddArchive(std::shared_ptr<Archive> archive) {
    if (!archive->IsLoaded()) {
        WriteLog("Attempting to add unloaded Archive at {} to Archive Manager", archive->GetPath());
        return nullptr;
    }
    ...
    WriteLog("Adding Archive {} to Archive Manager", archive->GetPath());
