...
Engine::MemoryStream::MemoryStream() {
...
Engine::MemoryStream::MemoryStream(char* nBuffer, size_t nBufferSize) : MemoryStream() {
    mBuffer = std::make_shared<std::vector<char>>(nBuffer, nBuffer + nBufferSize);
    mBufferSize = nBufferSize;
    mBaseAddress = 0;
}

Engine::MemoryStream::MemoryStream(std::shared_ptr<std::vector<char>> buffer) : MemoryStream() {
    mBuffer = buffer;
...
uint64_t Engine::MemoryStream::GetLength() {
    return mBuffer->size();
}

void Engine::MemoryStream::Seek(int32_t offset, SeekOffsetType seekType) {
...
std::unique_ptr<char[]> Engine::MemoryStream::Read(size_t length) {
...
void Engine::MemoryStream::Read(const char* dest, size_t length) {
    memcpy_s((void*)dest, length, &mBuffer->at(mBaseAddress), length);
    mBaseAddress += length;
}

int8_t Engine::MemoryStream::ReadByte() {
    return mBuffer->at(mBaseAddress++);
}

void Engine::MemoryStream::Write(char* srcBuffer, size_t length) {
...
void Engine::MemoryStream::WriteByte(int8_t value) {
...
std::vector<char> Engine::MemoryStream::ToVector() {
    return *mBuffer;
}

void Engine::MemoryStream::Flush() {
}

void Engine::MemoryStream::Close() {
}
