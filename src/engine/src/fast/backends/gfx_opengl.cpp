...
std::optional<std::string> opengl_include_fs(const std::string& path) {
    ...
    auto res = std::static_pointer_cast<Engine::Shader>(
        Engine::Context::GetInstance()->GetResourceManager()->LoadResource(path, true, init));
    if (res == nullptr) {
        return std::nullopt;
    }
...
std::string GfxRenderingAPIOGL::BuildFsShader(const CCFeatures& cc_features) {
    ...
    init->ByteOrder = Engine::Endianness::Native;
    init->Format = RESOURCE_FORMAT_BINARY;
    auto res = std::static_pointer_cast<Engine::Shader>(Engine::Context::GetInstance()->GetResourceManager()->LoadResource(
...
void GfxRenderingAPIOGL::DrawTriangles(float buf_vbo[], size_t buf_vbo_len, size_t buf_vbo_num_tris) {
    ...
    if (mCurrentZmodeDecal != mLastZmodeDecal) {
        ...
        if (mCurrentZmodeDecal) {
            ...
            GLfloat SSDB = -2;
            switch (Engine::Context::GetInstance()->GetConsoleVariables()->GetInteger(CVAR_Z_FIGHTING_MODE, 0)) {
