...
std::string Interpreter::GetBaseTexturePath(const std::string& path) {
    if (path.starts_with(Engine::IResource::gAltAssetPrefix)) {
        return path.substr(Engine::IResource::gAltAssetPrefix.length());
    }
    ...
    */
    ...
    if ((bool)gfx_check_image_signature((char*)data)) {
        std::shared_ptr<Fast::Texture> tex = std::static_pointer_cast<Fast::Texture>(
            Engine::Context::GetInstance()->GetResourceManager()->LoadResourceProcess((char*)data));
        texFlags = tex->Flags;
...
bool gfx_mtx_otr_filepath_handler_custom_f3dex2(F3DGfx** cmd0) {
    ...
    F3DGfx* cmd = *cmd0;
    const char* fileName = (const char*)cmd->words.w1;
    const int32_t* mtx = (const int32_t*)Engine::Context::GetInstance()->GetResourceManager()->GetResourceRawPointer(
        (const char*)fileName);
...
bool gfx_mtx_otr_handler_custom_f3dex2(F3DGfx** cmd0) {
    ...
    const int32_t* mtx =
        (const int32_t*)Engine::Context::GetInstance()->GetResourceManager()->GetResourceRawPointer(hash);
...
bool gfx_movemem_handler_otr(F3DGfx** cmd0) {
    ...
    if (ucode_handler_index == ucode_f3dex2) {
        gfx->GfxSpMovememF3dex2(index, offset,
                                Engine::Context::GetInstance()->GetResourceManager()->GetResourceRawPointer(hash));
    } else {
        auto light = (Fast::LightEntry*)Engine::Context::GetInstance()->GetResourceManager()->GetResourceRawPointer(hash);
...
int16_t Interpreter::CreateShader(const std::string& path) {
    ...
    if (path.rfind("GAME_WATER_", 0) == 0) {
        ...
        auto shader = Engine::Context::GetInstance()->GetResourceManager()->GetArchiveManager()->LoadFile(path);
...
const std::string* gfx_get_custom_shader_source(int16_t shaderId) {
    const std::shared_ptr<Interpreter> interpreter = mInstance.lock();
    if (!interpreter || shaderId < 0 || static_cast<size_t>(shaderId) >= interpreter->shader_ids.size()) {
        return nullptr;
    }
...
bool gfx_vtx_hash_handler_custom(F3DGfx** cmd0) {
    ...
    if (offset > 0xFFFFF) {
        ...
        F3DVtx* vtx = (F3DVtx*)Engine::Context::GetInstance()->GetResourceManager()->GetResourceRawPointer(hash);
...
bool gfx_vtx_otr_filepath_handler_custom(F3DGfx** cmd0) {
    ...
    F3DVtx* vtx =
        (F3DVtx*)Engine::Context::GetInstance()->GetResourceManager()->GetResourceRawPointer((const char*)fileName);
    vtx += vtxDataOff;
...
bool gfx_dl_otr_filepath_handler_custom(F3DGfx** cmd0) {
    ...
    char* fileName = (char*)cmd->words.w1;
    F3DGfx* nDL =
        (F3DGfx*)Engine::Context::GetInstance()->GetResourceManager()->GetResourceRawPointer((const char*)fileName);
...
bool gfx_dl_otr_hash_handler_custom(F3DGfx** cmd0) {
    ...
    if (C0(16, 1) == 0) {
        ...
        F3DGfx* gfx = (F3DGfx*)Engine::Context::GetInstance()->GetResourceManager()->GetResourceRawPointer(hash);
...
bool gfx_set_timg_handler_rdp(F3DGfx** cmd0) {
    ...
    if ((i & 1) != 1) {
        if (gfx_check_image_signature(imgData) == 1) {
            std::shared_ptr<Fast::Texture> tex = std::static_pointer_cast<Fast::Texture>(
                Engine::Context::GetInstance()->GetResourceManager()->LoadResourceProcess(imgData));
...
bool gfx_set_timg_otr_hash_handler_custom(F3DGfx** cmd0) {
    ...
    const char* fileName = Engine::Context::GetInstance()->GetResourceManager()->GetArchiveManager()->HashToCString(hash);
    uint32_t texFlags = 0;
    RawTexMetadata rawTexMetadata = {};
    ...
    std::shared_ptr<Fast::Texture> texture =
        std::static_pointer_cast<Fast::Texture>(Engine::Context::GetInstance()->GetResourceManager()->LoadResourceProcess(
            Engine::Context::GetInstance()->GetResourceManager()->GetArchiveManager()->HashToCString(hash)));
    if (texture != nullptr) {
        ...
        WriteLog("G_SETTIMG_OTR_HASH: Texture is null");
    }
...
bool gfx_set_timg_otr_filepath_handler_custom(F3DGfx** cmd0) {
    ...
    std::shared_ptr<Fast::Texture> texture = std::static_pointer_cast<Fast::Texture>(
        Engine::Context::GetInstance()->GetResourceManager()->LoadResourceProcess(fileName));
    if (texture != nullptr) {
        Interpreter* gfx = mInstance.lock().get();
        texFlags = texture->Flags;
        rawTexMetadata.width = texture->Width;
        ...
        WriteLog("G_SETTIMG_OTR_FILEPATH: Texture is null");
    }
    return false;
}
...
static void gfx_step() {
    ...
    if (cmd->words.trace.valid &&
        Engine::Context::GetInstance()->GetConsoleVariables()->GetInteger("gEnableGFXTrace", 0)) {
#define TRACE                                  \
    "\n====================================\n" \
    " - CMD: {:02X}\n"                         \
    " - Path: {}:{}\n"                         \
...
void Interpreter::InitializeRenderer(GfxWindowBackend* window,
    ...
    mMsaaLevel = Engine::Context::GetInstance()->GetConsoleVariables()->GetInteger(CVAR_MSAA_VALUE, 1);
...
void Interpreter::Run(Gfx* commands, const std::unordered_map<Mtx*, MtxF>& mtx_replacements) {
    ...
    auto dbg = Engine::Context::GetInstance()->GetGfxDebugger();
...
int32_t gfx_check_image_signature(const char* imgData) {
    ...
    if (i != 0) {
        return Engine::Context::GetInstance()->GetResourceManager()->OtrSignatureCheck(imgData);
    }
...
void Interpreter::RegisterBlendedTexture(const char* name, uint8_t* mask, uint8_t* replacement) {
    ...
    if (gfx_check_image_signature(reinterpret_cast<char*>(replacement))) {
        Fast::Texture* tex = std::static_pointer_cast<Fast::Texture>(
                                 Engine::Context::GetInstance()->GetResourceManager()->LoadResourceProcess(
