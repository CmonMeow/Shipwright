...
#include "rendering/backends/gfx_rendering_api.h"
...
Interpreter::Interpreter(Engine::ResourceManager& resources, Engine::ConsoleVariable& variables, GfxDebugger& debugger)
    : mResources(resources), mVariables(variables), mDebugger(debugger) {
    mRsp = new RSP();
    mRdp = new RDP();
...
Engine::ResourceManager& Interpreter::Resources() {
    return mResources;
}

Engine::ConsoleVariable& Interpreter::Variables() {
    return mVariables;
}
...
void Interpreter::ImportTextureRgba16(int tile, bool importReplacement) {
    ...
    const uint8_t* addr =
        importReplacement && (metadata->resource != nullptr)
            ? mMaskedTextures.find(metadata->resource->GetInitData()->Path)->second.replacementData
...
void Interpreter::ImportTextureRaw(int tile, bool importReplacement) {
    ...
    uint16_t width = metadata->width;
    uint16_t height = metadata->height;
    Engine::Rendering::TextureType type = metadata->type;
    std::shared_ptr<Engine::Rendering::Texture> resource = metadata->resource;

    // if texture type is CI4 or CI8 we need to apply tlut to it
    switch (type) {
        case Engine::Rendering::TextureType::Palette4bpp:
            ImportTextureCi4(tile, importReplacement);
            return;
        case Engine::Rendering::TextureType::Palette8bpp:
    ...
    switch (fmt) {
        case G_IM_FMT_RGBA:
            if (siz == G_IM_SIZ_16b) {
                ...
                WriteLog("RGBA Texture that isn't 16 or 32 bit. Size = {}", siz);
                // TODO: This can be reached by malformed display lists or
        ...
        case G_IM_FMT_IA:
            if (siz == G_IM_SIZ_4b) {
                ...
                WriteLog("IA Texture that isn't 4, 8, or 16 bit. Size = {}", siz);
                ;
            }
            break;
        case G_IM_FMT_CI:
            if (siz == G_IM_SIZ_4b) {
                ...
                WriteLog("CI Texture that isn't 4 or 8 bit. Size = {}", siz);
            }
            break;
        case G_IM_FMT_I:
            if (siz == G_IM_SIZ_4b) {
                ...
                WriteLog("I Texture that isn't 4 or 8 bit. Size = {}", siz);
            }
            break;
        case G_IM_FMT_YUV:
            WriteLog("YUV Textures not supported");
            break;
        default:
            WriteLog("Invalid texture format. Fmt = {}", fmt);
            break;
    }
}
...
void Interpreter::ImportTextureMask(int i, int tile) {
    ...
    auto maskIter = mMaskedTextures.find(metadata.resource->GetInitData()->Path);
    ...
    for (size_t i = 0; i < n_vertices; i++, dest_index++) {
        ...
        if (mRsp->geometry_mode & G_LIGHTING) {
            ...
            if (mRsp->geometry_mode & G_TEXTURE_GEN) {
                ...
                dotx = Engine::Math::clamp(dotx, -1.0f, 1.0f);
                doty = Engine::Math::clamp(doty, -1.0f, 1.0f);
        ...
        if (mRsp->geometry_mode & G_FOG) {
            ...
            fog_z = Engine::Math::clamp(fog_z, 0.0f, 255.0f);
...
void Interpreter::GfxSpTri1(uint8_t vtx1_idx, uint8_t vtx2_idx, uint8_t vtx3_idx, bool is_rect) {
    ...
    if (mRdp->current_shader.enabled) {
        const std::string* customShader = gfx_get_custom_shader_source(mRdp->current_shader.id);
        forceTwoSided = customShader != nullptr && customShader->rfind("GAME_WATER_", 0) == 0;
    }

    // game's water pass disables face culling. Fast3D performs culling
    // before OpenGL sees the triangles, so bypassing GL_CULL_FACE alone is not
    // sufficient to make the surface visible from above and below.
    if (!forceTwoSided && (mRsp->geometry_mode & cull_both) != 0) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (forceTwoSided) {
            // Native game water UVs are independent of N64 texture
            // state; they index the renderer-owned map mask and simulation.
...
void Interpreter::AdjustVIewportOrScissor(XYWidthHeight* area) {
    if (!mFbActive) {
        ...
        if (!mRendersToFb || (mMsaaLevel > 1 && mCurDimensions.width == mPresentationViewport.width &&
                              mCurDimensions.height == mPresentationViewport.height)) {
            area->x += mPresentationViewport.x;
            area->y += mGfxCurrentWindowDimensions.height -
                       (mPresentationViewport.y + mPresentationViewport.height);
        }
    } else {
...
void Interpreter::GfxDpLoadBlock(uint8_t tile, uint32_t uls, uint32_t ult, uint32_t lrs, uint32_t dxt) {
    ...
    const std::string& texPath =
        mRdp->texture_to_load.raw_tex_metadata.resource != nullptr
            ? mRdp->texture_to_load.raw_tex_metadata.resource->GetInitData()->Path
            : "";
...
void Interpreter::GfxDpFillRectangle(int32_t ulx, int32_t uly, int32_t lrx, int32_t lry) {
    ...
    // TODO: Replace this widescreen screen-fade workaround.
...
void Interpreter::Gfxs2dexBgCopy(F3DuObjBg* bg) {
    ...
    */
    ...
    if ((bool)gfx_check_image_signature((char*)data)) {
        std::shared_ptr<Engine::Rendering::Texture> tex = std::static_pointer_cast<Engine::Rendering::Texture>(
            mResources.LoadResourceProcess((char*)data));
        texFlags = tex->Flags;
...
void Interpreter::Gfxs2dexBg1cyc(F3DuObjBg* bg) {
    ...
    int16_t uls = bg->b.imageX >> 2;
    int16_t lrs = bg->b.imageW >> 2;

    int16_t dsdxRect = 1 << 10;
    int16_t ulsRect = bg->b.imageX << 3;
    // Flip flag only flips horizontally
    if (bg->b.imageFlip == G_BG_FLAG_FLIPS) {
        dsdxRect = -dsdxRect;
...
void Interpreter::Gfxs2dexRecyCopy(F3DuObjSprite* spr) {
    int16_t dsdx = 4 << 10;
    [[maybe_unused]] int16_t uls = spr->s.objX << 3;
    // Flip flag only flips horizontally
    ...
    GfxDpTextureRectangle(realX << 2, realY << 2, testX << 2, testY << 2, G_TX_RENDERTILE,
                          (int32_t)mRdp->texture_tile[0].uls << 3, (int32_t)mRdp->texture_tile[0].ult << 3,
...
bool gfx_marker_handler_resource(F3DGfx** cmd0) {
    Interpreter* gfx = mInstance;
    (*cmd0)++;
    F3DGfx* cmd = (*cmd0);
    gfx->mMarkerOn = true;
...
bool gfx_noop_handler_f3dex2(F3DGfx** cmd0) {
    ...
    if (p == 7) {
        ...
        if (g_exec_stack.disp_stack.size() == 0) {
            WriteLog("CLOSE_DISPS without matching open {}:{}", p, l);
        } else {
...
bool gfx_mtx_resource_path_handler_custom_f3dex2(F3DGfx** cmd0) {
    Interpreter* gfx = mInstance;
    F3DGfx* cmd = *cmd0;
    const char* fileName = (const char*)cmd->words.w1;
    const int32_t* mtx = (const int32_t*)gfx->Resources().GetResourceRawPointer(
        (const char*)fileName);
...
bool gfx_mtx_resource_path_handler_custom_f3d(F3DGfx** cmd0) {
    Interpreter* gfx = mInstance;
    F3DGfx* cmd = *cmd0;
    const char* fileName = (const char*)cmd->words.w1;
    const int32_t* mtx = (const int32_t*)gfx->Resources().GetResourceRawPointer(
...
bool gfx_mtx_resource_path_handler_custom(F3DGfx** cmd0) {
    if (ucode_handler_index == ucode_f3dex2) {
        return gfx_mtx_resource_path_handler_custom_f3dex2(cmd0);
    } else {
        return gfx_mtx_resource_path_handler_custom_f3d(cmd0);
    }
}

bool gfx_mtx_resource_handler_custom_f3dex2(F3DGfx** cmd0) {
    (*cmd0)++;
    F3DGfx* cmd = *cmd0;

    const uint64_t hash = ((uint64_t)cmd->words.w0 << 32) + cmd->words.w1;
    const int32_t* mtx =
        (const int32_t*)mInstance->Resources().GetResourceRawPointer(hash);
...
bool gfx_mtx_resource_handler_custom_f3d(F3DGfx** cmd0) {
    Interpreter* gfx = mInstance;
    (*cmd0)++;
    F3DGfx* cmd = *cmd0;
    ...
    const uint64_t hash = ((uint64_t)cmd->words.w0 << 32) + cmd->words.w1;
    const int32_t* mtx =
        (const int32_t*)gfx->Resources().GetResourceRawPointer(hash);
    if (mtx != nullptr) {
        cmd--;
        gfx->GfxSpMatrix(C0(16, 8), mtx);
        cmd++;
...
bool gfx_mtx_resource_handler_custom(F3DGfx** cmd0) {
    if (ucode_handler_index == ucode_f3dex2) {
        return gfx_mtx_resource_handler_custom_f3dex2(cmd0);
    } else {
        return gfx_mtx_resource_handler_custom_f3d(cmd0);
    }
}
...
bool gfx_movemem_handler_resource(F3DGfx** cmd0) {
    Interpreter* gfx = mInstance;
    F3DGfx* cmd = *cmd0;
    ...
    if (ucode_handler_index == ucode_f3dex2) {
        gfx->GfxSpMovememF3dex2(index, offset,
                                gfx->Resources().GetResourceRawPointer(hash));
    } else {
        auto light = (Engine::Rendering::LightEntry*)gfx->Resources().GetResourceRawPointer(hash);
...
int16_t Interpreter::CreateShader(const std::string& path) {
    std::string source;
    // Built-in renderer effects use a stable marker instead of requiring a
    // second asset archive just to select native backend code.
    if (path.rfind("GAME_WATER_", 0) == 0) {
        source = path;
    } else {
        std::shared_ptr<Engine::ResourceInitData> initData = std::make_shared<Engine::ResourceInitData>();
        initData->Path = path;
        initData->IsCustom = false;
        initData->ByteOrder = Engine::Endianness::Native;
        auto shader = mResources.GetArchiveManager()->LoadFile(path);
        if (shader == nullptr || !shader->IsLoaded) {
            return -1;
        }
        source = std::string(shader->Buffer->data());
    }

    const auto existing = std::find(shader_ids.begin(), shader_ids.end(), source);
    if (existing != shader_ids.end()) {
        return static_cast<int16_t>(std::distance(shader_ids.begin(), existing));
    }
    if (shader_ids.size() >= INT16_MAX) {
        return -1;
    }
    shader_ids.push_back(std::move(source));
    return shader_ids.size() - 1;
}

const std::string* gfx_get_custom_shader_source(int16_t shaderId) {
    const Interpreter* interpreter = mInstance;
    if (interpreter == nullptr || shaderId < 0 || static_cast<size_t>(shaderId) >= interpreter->shader_ids.size()) {
        return nullptr;
    }
    return &interpreter->shader_ids[shaderId];
}
...
bool gfx_set_shader_custom(F3DGfx** cmd0) {
    ...
    if (file == nullptr) {
        gfx->mRdp->current_shader = { false, -1, false };
        return false;
    }

    std::string path(file);
    if (path == "GAME_WATER") {
        path += "_" + std::to_string((uint8_t)C0(16, 1));
    }
    const auto shaderId = gfx->CreateShader(path);
    gfx->mRdp->current_shader = { shaderId >= 0, shaderId, (uint8_t)C0(16, 1) };
    return false;
}
...
bool gfx_vtx_hash_handler_custom(F3DGfx** cmd0) {
    ...
    if (offset > 0xFFFFF) {
        ...
        F3DVtx* vtx = (F3DVtx*)gfx->Resources().GetResourceRawPointer(hash);
...
bool gfx_vtx_resource_path_handler_custom(F3DGfx** cmd0) {
    Interpreter* gfx = mInstance;
    F3DGfx* cmd = *cmd0;
    ...
    F3DVtx* vtx =
        (F3DVtx*)gfx->Resources().GetResourceRawPointer((const char*)fileName);
    vtx += vtxDataOff;
...
bool gfx_dl_resource_path_handler_custom(F3DGfx** cmd0) {
    F3DGfx* cmd = *cmd0;
    char* fileName = (char*)cmd->words.w1;
    F3DGfx* nDL = (F3DGfx*)mInstance->Resources().GetResourceRawPointer((const char*)fileName);
...
bool gfx_dl_resource_hash_handler_custom(F3DGfx** cmd0) {
    F3DGfx* cmd = *cmd0;
    if (C0(16, 1) == 0) {
        // Push return address
        (*cmd0)++;
        ...
        F3DGfx* gfx = (F3DGfx*)mInstance->Resources().GetResourceRawPointer(hash);
...
// TODO: Handle special resource opcodes.
...
// TODO: Handle special resource opcodes.
bool gfx_branch_z_resource_handler_f3dex2(F3DGfx** cmd0) {
    // Push return address
    Interpreter* gfx = mInstance;
    F3DGfx* cmd = (*cmd0);
...
bool gfx_tri1_resource_handler_f3dex2(F3DGfx** cmd0) {
    Interpreter* gfx = mInstance;
...
bool gfx_set_timg_handler_rdp(F3DGfx** cmd0) {
    ...
    if ((i & 1) != 1) {
        if (gfx_check_image_signature(imgData) == 1) {
            std::shared_ptr<Engine::Rendering::Texture> tex = std::static_pointer_cast<Engine::Rendering::Texture>(
                gfx->Resources().LoadResourceProcess(imgData));
...
bool gfx_set_timg_resource_hash_handler_custom(F3DGfx** cmd0) {
    uintptr_t addr = (*cmd0)->words.w1;
    (*cmd0)++;
    uint64_t hash = ((uint64_t)(*cmd0)->words.w0 << 32) + (uint64_t)(*cmd0)->words.w1;

    const char* fileName = mInstance->Resources().GetArchiveManager()->HashToCString(hash);
    uint32_t texFlags = 0;
    RawTexMetadata rawTexMetadata = {};
    ...
    std::shared_ptr<Engine::Rendering::Texture> texture =
        std::static_pointer_cast<Engine::Rendering::Texture>(mInstance->Resources().LoadResourceProcess(
            mInstance->Resources().GetArchiveManager()->HashToCString(hash)));
    if (texture != nullptr) {
        ...
        // TODO: Caching is disabled to avoid HD-texture corruption. Profile the uncached path before deciding
        // whether to repair and restore the cache.
        ...
        WriteLog("G_SETTIMG_RESOURCE_HASH: texture is null");
    }
...
bool gfx_set_timg_resource_path_handler_custom(F3DGfx** cmd0) {
    F3DGfx* cmd = *cmd0;
    ...
    std::shared_ptr<Engine::Rendering::Texture> texture = std::static_pointer_cast<Engine::Rendering::Texture>(
        mInstance->Resources().LoadResourceProcess(fileName));
    if (texture != nullptr) {
        Interpreter* gfx = mInstance;
        texFlags = texture->Flags;
        rawTexMetadata.width = texture->Width;
        ...
        WriteLog("G_SETTIMG_RESOURCE_PATH: texture is null");
    }
    return false;
}
...
bool gfx_register_blended_texture_handler_custom(F3DGfx** cmd0) {
    ...
    if (!gfx_check_image_signature(timg)) {
        WriteLog(
            "G_REGBLENDEDTEX: texture does not contain a valid resource path");
        return false;
    }
...
static constexpr UcodeHandler resourceHandlers = {
    { RESOURCE_G_SETTIMG_HASH, { "G_SETTIMG_RESOURCE_HASH", gfx_set_timg_resource_hash_handler_custom } },
    { RESOURCE_G_SETFB, { "G_SETFB", gfx_set_fb_handler_custom } },
    { RESOURCE_G_RESETFB, { "G_RESETFB", gfx_reset_fb_handler_custom } },
    { RESOURCE_G_SETTIMG_FB, { "G_SETTIMG_FB", gfx_set_timg_fb_handler_custom } },
    { RESOURCE_G_VTX_PATH, { "G_VTX_RESOURCE_PATH", gfx_vtx_resource_path_handler_custom } },
    { RESOURCE_G_SETTIMG_PATH, { "G_SETTIMG_RESOURCE_PATH", gfx_set_timg_resource_path_handler_custom } },
    { RESOURCE_G_TRI1, { "G_TRI1_RESOURCE", gfx_tri1_resource_handler_f3dex2 } },
    { RESOURCE_G_DL_PATH, { "G_DL_RESOURCE_PATH", gfx_dl_resource_path_handler_custom } },
    { RESOURCE_G_PUSHCD, { "G_PUSHCD", gfx_pushcd_handler_custom } },
    { RESOURCE_G_MTX_PATH, { "G_MTX_RESOURCE_PATH", gfx_mtx_resource_path_handler_custom } },
    { RESOURCE_G_DL_HASH, { "G_DL_RESOURCE_HASH", gfx_dl_resource_hash_handler_custom } },
    { RESOURCE_G_VTX_HASH, { "G_VTX_RESOURCE_HASH", gfx_vtx_hash_handler_custom } },
    { RESOURCE_G_MARKER, { "G_MARKER", gfx_marker_handler_resource } },
    { RESOURCE_G_INVALTEXCACHE, { "G_INVALTEXCACHE", gfx_invalidate_tex_cache_handler_f3dex2 } },
    { RESOURCE_G_BRANCH_Z, { "G_BRANCH_Z_RESOURCE", gfx_branch_z_resource_handler_f3dex2 } },
    { RESOURCE_G_MTX, { "G_MTX_RESOURCE", gfx_mtx_resource_handler_custom } },
    { RESOURCE_G_TEXRECT_WIDE, { "G_TEXRECT_WIDE", gfx_tex_rect_wide_handler_custom } },
    { RESOURCE_G_FILLWIDERECT, { "G_FILLWIDERECT", gfx_fill_wide_rect_handler_custom } },
    { RESOURCE_G_SETGRAYSCALE, { "G_SETGRAYSCALE", gfx_set_grayscale_handler_custom } },
    { RESOURCE_G_EXTRAGEOMETRYMODE,
      { "G_EXTRAGEOMETRYMODE", gfx_extra_geometry_mode_handler_custom } },
    { RESOURCE_G_COPYFB, { "G_COPYFB", gfx_copy_fb_handler_custom } },
    { RESOURCE_G_IMAGERECT, { "G_IMAGERECT", gfx_image_rect_handler_custom } },
    { RESOURCE_G_DL_INDEX, { "G_DL_INDEX", gfx_dl_index_handler } },
    { RESOURCE_G_READFB, { "G_READFB", gfx_read_fb_handler_custom } },
    { RESOURCE_G_REGBLENDEDTEX,
      { "G_REGBLENDEDTEX", gfx_register_blended_texture_handler_custom } },
    { RESOURCE_G_SETINTENSITY, { "G_SETINTENSITY", gfx_set_intensity_handler_custom } },
    { RESOURCE_G_MOVEMEM_HASH, { "G_MOVEMEM_RESOURCE_HASH", gfx_movemem_handler_resource } },
    { RESOURCE_G_LOAD_SHADER, { "G_LOAD_SHADER", gfx_set_shader_custom } },
};
...
const char* GfxGetOpcodeName(int8_t opcode) {
    if (resourceHandlers.contains(opcode)) {
        return resourceHandlers.at(opcode).first;
    }
    ...
    if (ucode_handler_index < ucode_handlers.size()) {
        if (ucode_handlers[ucode_handler_index]->contains(opcode)) {
            ...
            WriteLog("Unhandled OP code: 0x{:X}, for loaded ucode: {}", (uint8_t)opcode,
                            (uint32_t)ucode_handler_index);
        }
    } else {
        WriteLog("Unhandled OP code: 0x{:X}, invalid ucode: {}", (uint8_t)opcode, (uint32_t)ucode_handler_index);
    }
...
static void gfx_step() {
    ...
    if (cmd->words.trace.valid &&
        mInstance->Variables().GetInteger("gEnableGFXTrace", 0)) {
#define TRACE                                  \
    "\n====================================\n" \
    " - CMD: {:02X}\n"                         \
    " - Path: {}:{}\n"                         \
    ...
    " - W1: {:08X}\n"                          \
    "===================================="
        WriteLog(TRACE, (uint8_t)opcode, cmd->words.trace.file, cmd->words.trace.idx, cmd->words.w0, cmd->words.w1);
    }
#endif
    ...
    if (resourceHandlers.contains(opcode)) {
        if (resourceHandlers.at(opcode).second(&cmd)) {
            return;
        }
...
void Interpreter::InitializeRenderer(OpenGLPresentation* presentation,
    ...
    mCurDimensions.internal_mul =
        mVariables.GetFloat(CVAR_INTERNAL_RESOLUTION, 1);
    mMsaaLevel = mVariables.GetInteger(CVAR_MSAA_VALUE, 1);
...
void Interpreter::ReleaseGraphicsResources() {
...
bool Interpreter::ViewportMatchesRendererResolution() const {
#ifdef __APPLE__
    // Always treat the viewport as not matching the render resolution on mac
    // to avoid issues with retina scaling.
    return false;
#else
    if (mCurDimensions.width == mPresentationViewport.width &&
        mCurDimensions.height == mPresentationViewport.height) {
        return true;
    }
    return false;
...
void Interpreter::StartFrame() {
    mGfxCurrentWindowDimensions.width = mPresentation->Width();
    mGfxCurrentWindowDimensions.height = mPresentation->Height();
...
void Interpreter::EndFrame() {
    mRapi->EndFrame();
    mPresentation->SwapBuffersBegin();
    mRapi->FinishRender();
    mPresentation->SwapBuffersEnd();
}
...
void Interpreter::CopyFrameBuffer(int fb_dst_id, int fb_src_id, bool copyOnce, bool* hasCopiedPtr) {
    ...
    // then the source coordinates must account for any docked viewport elements
    if (fb_src_id == 0 || (mMsaaLevel > 1 && mCurDimensions.width == mPresentationViewport.width &&
                           mCurDimensions.height == mPresentationViewport.height)) {
        srcX0 = mPresentationViewport.x;
        srcY0 = mPresentationViewport.y;
        srcX1 = mPresentationViewport.x + mPresentationViewport.width;
        srcY1 = mPresentationViewport.y + mPresentationViewport.height;
    } else {
        srcX0 = 0;
        srcY0 = 0;
...
int32_t gfx_check_image_signature(const char* imgData) {
    ...
    if (i != 0) {
        return mInstance != nullptr && mInstance->Resources().HasResourceSignature(imgData);
    }
...
void Interpreter::RegisterBlendedTexture(const char* name, uint8_t* mask, uint8_t* replacement) {
    ...
    if (gfx_check_image_signature(reinterpret_cast<char*>(replacement))) {
        Engine::Rendering::Texture* tex = std::static_pointer_cast<Engine::Rendering::Texture>(
                                 mResources.LoadResourceProcess(
...
extern "C" int gfx_create_framebuffer(uint32_t width, uint32_t height, uint32_t native_width, uint32_t native_height,
                                      uint8_t resize) {
    return Engine::Rendering::mInstance->CreateFrameBuffer(width, height, native_width, native_height, resize);
}

extern "C" void gfx_texture_cache_clear() {
    Engine::Rendering::mInstance->TextureCacheClear();
}
