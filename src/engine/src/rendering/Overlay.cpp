...
#include "rendering/OverlayFontData.h"
...
extern "C" void Overlay_QueueGameText(const char* text, float x, float y, float red, float green,
    ...
    Engine::Rendering::Overlay::QueueGameText(text, x, y, red, green, blue, alpha);
}

extern "C" const uint8_t* OverlayFont_GetSerifGlyph(unsigned char character) {
    return character >= 33 && character <= 127 ? Engine::Rendering::OverlayFont::Serif[character - 32]
                                                : Engine::Rendering::OverlayFont::Serif[0];
}
