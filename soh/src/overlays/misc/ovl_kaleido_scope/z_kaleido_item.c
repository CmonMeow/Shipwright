...
void KaleidoScope_DrawAmmoCount(PauseContext* pauseCtx, GraphicsContext* gfxCtx, s16 item, int slot) {
    ...
    if (i != 0) {
        gSPVertex(
            POLY_OPA_DISP++,
            &pauseCtx->itemVtx[(sAmmoVtxOffset[slot] + 31) * 4],
            4, 0);
    ...
    gSPVertex(POLY_OPA_DISP++, &pauseCtx->itemVtx[(sAmmoVtxOffset[slot] + 32) * 4], 4, 0);
...
void KaleidoScope_UpdateItemEquip(PlayState* play) {
    ...
    const s16 itemIconPositions[3][2] = {
        { C_LEFT_BUTTON_X, C_LEFT_BUTTON_Y },
        { C_DOWN_BUTTON_X, C_DOWN_BUTTON_Y },
        { C_RIGHT_BUTTON_X, C_RIGHT_BUTTON_Y },
    };

    for (s32 i = 0; i < ARRAY_COUNT(sCButtonPosX); i++) {
        sCButtonPosX[i] = OTRGetRectDimensionFromRightEdge(itemIconPositions[i][0]) - 160;
        sCButtonPosY[i] = 120 - itemIconPositions[i][1];
    }
